...
//yangmingjin@realme.com, 2020/05/19, Add for power debug
#ifdef VENDOR_EDIT
			power_helper_save_to_buf("active wakeup source: %s", ws->name);
#endif /* VENDOR_EDIT */
			active = 1;
		} else if (!active &&
			   (!last_activity_ws ||
...
//yangmingjin@realme.com, 2020/05/19, Add for power debug
#ifdef VENDOR_EDIT
		power_helper_save_to_buf("last active wakeup source: %s", last_activity_ws->name);
#endif /* VENDOR_EDIT */
	}
	srcu_read_unlock(&wakeup_srcu, srcuidx);
}
...
//yangmingjin@realme.com, 2020/05/19, Add for power debug
#ifdef VENDOR_EDIT
		power_helper_save_to_buf("%s", "PM: Wakeup pending, aborting suspend");
#endif /* VENDOR_EDIT */
