...
int FP_LDO_PowerOn_Fan53870(int ldo_num, int set_mv_avdd)
{
        printk("FP_LDO_PowerOn_Fan53870 enter!\n");
  	if (is_fan53870_pmic())
  	{
  		fan53870_cam_ldo_set_voltage(ldo_num, set_mv_avdd);
  		printk("fp set vdd successed!\n");
  	} else {
  		printk("fp set vdd on fail!\n");
  		return -1;
  	}
...
int gf_parse_dts(struct gf_dev* gf_dev)
{
    ...
    if(FP_GOODIX_5658 == get_fpsensor_type() ||
       ...
       FP_GOODIX_5298 == get_fpsensor_type()) { //add new fpsensor if needed
    ...
    if (IS_ERR(gf_dev->pinctrl)) {
        dev_err(&pdev->dev, "can not get the gf pinctrl, err_code: %d", gf_dev->pinctrl);
...
#if CONFIG_OPPO_FINGERPRINT_PLATFORM != 6853
 ...
 #if CONFIG_OPPO_FINGERPRINT_PLATFORM == 6873 || CONFIG_OPPO_FINGERPRINT_PLATFORM == 6853
...
int gf_power_on(struct gf_dev* gf_dev)
{
    ...
    for (index = 0; index < gf_dev->power_num; index++) {
		...
		rc = FP_LDO_PowerOn_Fan53870(7, 3300);
                if(rc < 0){
		     pr_err("fp 3.3v power on fail !\n");
		}
             }
