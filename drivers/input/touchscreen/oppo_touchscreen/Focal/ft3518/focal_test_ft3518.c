...
static void fts_test_save_data(char *name, int *data, int datacnt, int line, int fd)
{
    ...
    cnt += snprintf(data_buf + cnt, max_size - cnt, "%s\n", name);
    for (i = 0; i < datacnt; i++) {
...
static int fts_noise_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
    ...
    if (!ts_data->fts_autotest_offset->fts_noise_data_P || !ts_data->fts_autotest_offset->fts_noise_data_P) {
...
//    FTS_TEST_SAVE_INFO("Check Tx Linearity\n");
    ...
    for (i = 0; i < node_num; i++) {
        ...
        if ((rl_tmp[i] < thr->tx_linearity_min[i]) || (rl_tmp[i] > thr->tx_linearity_max[i])) {
            FTS_TEST_SAVE_ERR("test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
                              i / rx_num + 1, i % rx_num + 1, rl_tmp[i],
                              thr->tx_linearity_min[i], thr->tx_linearity_max[i]);
            result = false;
        }
    }
...
static int fts_scap_cb_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
    ...
    if (!ts_data->fts_autotest_offset->fts_scap_cb_data_P || !ts_data->fts_autotest_offset->fts_scap_cb_data_N) {
        TPD_INFO("fts_scap_cb_data_P || fts_scap_cb_data_N is NULL");
        return 0;
    }
...
static int fts_scap_rawdata_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
    ...
    if (!ts_data->fts_autotest_offset->fts_scap_raw_data_P || !ts_data->fts_autotest_offset->fts_scap_raw_data_N) {
        TPD_INFO("fts_scap_raw_data_P || fts_scap_raw_data_N is NULL");
        return 0;
    }
...
static int fts_panel_differ_autotest(struct fts_ts_data *ts_data, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int i = 0;
    ...
    if (!ts_data->fts_autotest_offset->fts_panel_differ_data_P || !ts_data->fts_autotest_offset->fts_panel_differ_data_N) {
        TPD_INFO("fts_panel_differ_data_P || fts_panel_differ_data_N is NULL");
        return 0;
    }
    if (!thr || !thr->node_valid || !thr->panel_differ_min || !thr->panel_differ_max) {
        FTS_TEST_SAVE_ERR("thr/node_valid/panel_differ_h_min/max is null\n");
        ret = -EINVAL;
        goto test_err;
    }
    ...
    for (i = 0; i < 3; i++) {
        ...
        rawdata_addr = FACTORY_REG_RAWDATA_ADDR_MC_SC;
        byte_num = node_num * 2;
        ret = read_rawdata(FACTORY_REG_LINE_ADDR, 0xAA, rawdata_addr, byte_num, ts_data->panel_differ_raw);
        if (ret < 0) {
            FTS_TEST_SAVE_ERR("read rawdata fail\n");
        }
    }
    ...
    for (i = 0; i < node_num; i++) {
        ts_data->panel_differ_raw[i] = ts_data->panel_differ_raw[i] / 10;
    }
    ...
    for (i = 0; i < node_num; i++) {
        ...
        if ((ts_data->panel_differ_raw[i] < ts_data->fts_autotest_offset->fts_panel_differ_data_N[i])
            || (ts_data->panel_differ_raw[i] > ts_data->fts_autotest_offset->fts_panel_differ_data_P[i])) {
            TPD_INFO("panel differ ERR [%d]: [%d] > [%d] > [%d] \n", i, ts_data->fts_autotest_offset->fts_panel_differ_data_P[i], ts_data->panel_differ_raw[i], ts_data->fts_autotest_offset->fts_panel_differ_data_N[i]);
            FTS_TEST_SAVE_ERR("test fail,node(%4d,%4d)=%5d,range=(%5d,%5d)\n",
                              i / rx_num + 1, i % rx_num + 1, ts_data->panel_differ_raw[i],
                              ts_data->fts_autotest_offset->fts_panel_differ_data_N[i], ts_data->fts_autotest_offset->fts_panel_differ_data_P[i]);
            tmp_result = false;
        }
    }
...
static int fts_start_test(struct fts_ts_data *ts_data)
{
    ...
    /* panel differ test */
    ret = fts_panel_differ_autotest(ts_data, &temp_result);
    if ((ret < 0) || (false == temp_result)) {
        test_result = false;
        test_result |= (1 << 7);
        failed_count += 1;
