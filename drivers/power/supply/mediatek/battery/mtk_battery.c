...
#ifndef CONFIG_OPPO_CHARGER_MATCH_MTKGAUGE
...
enum {
	...
	BAT_TYPE__GYU_4450mv, //260mV~320mV
	...
	BAT_TYPE__ATL_4450mv, //610mV~750mV
...
int battery_type_check(void)
{
	...
	if(!is_vooc_project()){
		...
		/* Yunqing.Wang@BSP.Kernel.Driver, 2020/06/22, for 20625 battery type */
		if (is_project(OPPO_20625) || is_project(OPPO_20626)) {
				if (value >= 200 && value <= 350){
							battery_type = BAT_TYPE__GYU_4450mv;
							gm.battery_id = 0;
...
#if defined CONFIG_OPPO_4450MV_BATTERY_SUPPORT
...
try_again:
	...
	if (battery_type == BAT_TYPE__GYU_4450mv || battery_type == BAT_TYPE__ATL_4450mv) {
		return true;
	} else {
		if (retry_flag == 0) {
			retry_flag = 1;
...
#elif defined CONFIG_OPPO_4400MV_BATTERY_SUPPORT
...
#ifdef CONFIG_OPPO_CHARGER_MTKAGING
