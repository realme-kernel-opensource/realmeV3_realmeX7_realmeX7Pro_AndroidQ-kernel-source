...
void fg_zcv_int_handler(void)
{
	...
	int zcv_intr_curr = 0;
	int zcv = 0;
	/* xiejianglong@BSP.CHG.Basic,2020/7/31,add for 20625cake ALPS05260024 mtk modify battery */	
	int tmp = force_get_tbat(true);
	...
	/* xiejianglong@BSP.CHG.Basic,2020/7/31,add for 20625cake ALPS05260024 mtk modify battery */
	bm_err("[%s] car:%d zcv_curr:%d zcv:%d, slp_cur_avg:%d tmp: %d\n",
		__func__,
		fg_coulomb, zcv_intr_curr, zcv,
		//fg_cust_data.sleep_current_avg);
		/* xiejianglong@BSP.CHG.Basic,2020/7/31,add for 20625cake ALPS05260024 mtk modify battery */		
		fg_cust_data.sleep_current_avg, tmp);

	//if (abs(zcv_intr_curr) < fg_cust_data.sleep_current_avg) {
	/* xiejianglong@BSP.CHG.Basic,2020/7/31,add for 20625cake ALPS05260024 mtk modify battery */
	if (abs(zcv_intr_curr) < fg_cust_data.sleep_current_avg && tmp > 15) {
