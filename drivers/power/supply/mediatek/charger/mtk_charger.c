			...
			struct charger_consumer *consumer, bool en)
{
	...
	if (en == consumer->hv_charging_disabled)
	...
	int idx, bool en)
{
	...
	if (en == consumer->power_path_disabled[idx])
		consumer->power_path_disabled[idx] = !en;
	else {
		pr_info("[%s] already set: %d %d\n", __func__,
			consumer->power_path_disabled[idx], en);
		return 0;
	...
	list_for_each(pos, phead) {
		ptr = container_of(pos, struct charger_consumer, list);
		if (ptr->power_path_disabled[idx] == true) {
			en = false;
			break;
		}
		if (list_is_last(pos, phead))
