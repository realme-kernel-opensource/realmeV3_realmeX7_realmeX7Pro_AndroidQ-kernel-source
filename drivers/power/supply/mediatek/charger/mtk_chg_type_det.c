...
extern void oppo_gauge_set_event(int event);
...
extern void switch_usb_state(int usb_state);//ouyangbaili for zola bring up
#endif /*VENDOR_EDIT*/
...
{
	pr_err("charger_in_work_handler\n");
...
/* shifan@bsp.tp 2019.1118 add for notifying tp when charging*/
			...
			switch_usb_state(1);//ouyangbaili for zola bring up
#endif
...
//add by lukaili notify fuelgauge for other charger
				fg_charger_in_handler();
				oppo_gauge_set_event(CHARGER_NOTIFY_START_CHARGING);
#endif
...
/* shifan@bsp.tp 2019.1118 add for notifying tp when charging*/
			...
			switch_usb_state(0);//ouyangbaili for zola bring up
#endif
...
//add by lukaili notify fuelgauge for other charger
		fg_charger_in_handler();
		oppo_gauge_set_event(CHARGER_NOTIFY_STOP_CHARGING);
#endif
