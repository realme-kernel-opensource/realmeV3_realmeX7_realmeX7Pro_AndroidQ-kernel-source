...
int oppo_usb_get_property(struct power_supply *psy,
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_OTG_ONLINE:
			...
			break;
		/*case POWER_SUPPLY_PROP_FLASH_SWITCH:
			val->intval = chip->flash_switch;
			break;*/
		default:
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_OTG_SWITCH:
			#ifdef VENDOR_EDIT/* OuYangBaiLi@BSP.CHG.Basic, 2020/08/19,Add for cannot detect headset when reboot */
			charger_xlog_printk(CHG_LOG_CRTI, "otg_switch: %d,intval: %d\n", chip->otg_switch,val->intval);
			if (val->intval == 1 && chip->otg_switch == false) {
				chip->otg_switch = true;
				oppo_set_otg_switch_status(true);
			} else if(val->intval == 0 && chip->otg_switch == true){
				chip->otg_switch = false;
				chip->otg_online = false;
				oppo_set_otg_switch_status(false);
			} else {
				charger_xlog_printk(CHG_LOG_CRTI, "not_set otg_switch: %d,intval: %d\n", chip->otg_switch,val->intval);
			}
			//charger_xlog_printk(CHG_LOG_CRTI, "set otg_switch: %d,intval: %d\n", chip->otg_switch,val->intval);
			charger_xlog_printk(CHG_LOG_CRTI, "otg_switch: %d\n", chip->otg_switch);
			#endif /* VENDOR_EDIT */
			break;
		/*case POWER_SUPPLY_PROP_FLASH_SWITCH:
			if (val->intval == 1) {
				chip->flash_switch = true;
			} else {
				chip->flash_switch = false;
			}
			#ifdef CONFIG_OPPO_CHARGER_MTK6889//for dali
			flash_switch_set();
			#endif
			charger_xlog_printk(CHG_LOG_CRTI, "flash_switch: %d\n", chip->flash_switch);
			break;*/
		default:
...
int oppo_chg_init(struct oppo_chg_chip *chip)
{
	...
	power_helper_charge_data_init(chip, oppo_chg_get_battery_data);
...
#ifdef CONFIG_HIGH_TEMP_VERSION
	chip->limits.hot_bat_decidegc = 990;
#endif
...
static void battery_temp_anti_shake_handle(struct oppo_chg_chip *chip)
{
	...
	//charger_xlog_printk(CHG_LOG_CRTI,"[oppo_chg_max],cold_bat_decidegc = %d,chip->limits.little_cold_bat_decidegc,chip->limits.cool_bat_decidegc,chip->limits.little_cool_bat_decidegc, \
	//chip->limits.normal_bat_decidegc,chip->limits.warm_bat_decidegc,chip->limits.hot_bat_decidegc\n",
	//	chip->limits.cold_bat_decidegc,
	//	chip->limits.little_cold_bat_decidegc,
	//	chip->limits.cool_bat_decidegc,
	//	chip->limits.little_cool_bat_decidegc,
...
static void oppo_chg_check_tbatt_normal_status(struct oppo_chg_chip *chip)
{
	...
	if (chip->tbatt_normal_pre_shake == TBATT_PRE_SHAKE_INVALID) {
		chip->tbatt_pre_shake = batt_temp;
		#ifdef VENDOR_EDIT
		/* xiejianglong@BSP.CHG.Basic,2020/7/23,add for 20625cake temp normal anti shake */
	...
	charger_xlog_printk(CHG_LOG_CRTI, "tbatt_normal_status: %d\n", chip->tbatt_normal_status);
}
...
static void oppo_chg_update_ui_soc(struct oppo_chg_chip *chip)
{
	...
	if (chip->charger_exist && chip->batt_exist
		...
		soc_up_count = 0;
		#ifdef CONFIG_HIGH_TEMP_VERSION
		if(chip->soc <= 5 && chip->batt_volt >= 3500) {
			chip->soc = 5;//for aging test
                  	charger_xlog_printk(CHG_LOG_CRTI,
				"keep ui_soc:%d, soc:%d, batt_volt:%d \n",
				chip->ui_soc, chip->soc,chip->batt_volt);
                }
		#endif
...
static void oppo_chg_check_status_full(struct oppo_chg_chip *chip)
{
	...
	if ((is_batt_full == 1) || (chip->charging_state == CHARGING_STATUS_FULL)
		...
		if (chip->recharge_after_full == true || chip->vbatt_num == 2) {
			chip->dod0_counts ++;
			if (chip->dod0_counts == DOD0_COUNTS && chip->temperature <= 350 && chip->vbatt_num == 2) {
				if (chip->vbatt_num == 2) {
