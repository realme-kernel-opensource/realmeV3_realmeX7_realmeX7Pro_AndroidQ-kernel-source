...
//====================================================================//
#endif /* VENDOR_EDIT */
#ifdef VENDOR_EDIT/* YanGang@BSP.CHG.Basic, 2019/12/24, add for ccdetect gpio */
...
bool oppo_ccdetect_support_check(void);
int oppo_chg_ccdetect_parse_dt(struct oppo_chg_chip *chip);
//====================================================================//
#endif /* VENDOR_EDIT */
...
//delete mt6360 shipmode begin
//extern int charger_dev_enable_pmic_shipmode(struct charger_device *charger_dev);

int oppo_mt6360_ship_mode_enable(void)
{
	//struct charger_device *shipmode_chg_dev = NULL;

	//if (!pinfo->chg1_dev) {
	//	printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
	//	return 0;
	//}
	//shipmode_chg_dev = pinfo->chg1_dev;
	//charger_dev_enable_pmic_shipmode(shipmode_chg_dev);

	return 0;
}
//delete mt6360 shipmode end
...
static void set_usbswitch_to_rxtx(struct oppo_chg_chip *chip,int value)
{
	int ret = 0;
	...
	//	return;
	//}
	printk(KERN_ERR "[OPPO_CHG][%s]: value:%d\n", __func__,value);
	if(value){
...
void mt_set_chargerid_switch_val(int value)
{
	...
	set_usbswitch_to_rxtx(chip,value);
	printk(KERN_ERR "[OPPO_CHG][%s]: value:%d\n", __func__,value);
...
static void smbchg_enter_shipmode(struct oppo_chg_chip *chip)
{
	...
	if (oppo_ship_check_is_gpio(chip) == true) {
		...
		oppo_mt6360_ship_mode_enable();
		chg_err("power off after 15s\n");
	}
}
void enter_ship_mode_function(void)
{
	if (!g_oppo_chip) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chip not ready!\n", __func__);
		return;
					...
					int *temp)
{
	...
	for (i = ARRAY_SIZE(con_volt_20518) - 1; i >= 0; i--) {
		if (con_volt_20518[i] >= charger_ntc_volt)
			break;
		else if (i == 0)
			break;
	}
	*temp = con_temp_20518[i]; 
    *temp = *temp * 1000;
	//printk("hupeng9999 temp:%d\n ",*temp);
	return 0;
}
...
bool oppo_ccdetect_check_is_gpio(struct oppo_chg_chip *chip)
{
	...
	if (gpio_is_valid(chip->chgic_mtk.oppo_info->ccdetect_gpio)){
...
int oppo_ccdetect_gpio_init(struct oppo_chg_chip *chip)
{
	...
	chip->chgic_mtk.oppo_info->pinctrl = devm_pinctrl_get(chip->dev);

	if (IS_ERR_OR_NULL(chip->chgic_mtk.oppo_info->pinctrl)) {
		chg_err("get ccdetect_pinctrl fail\n");
		return -EINVAL;
	}

	chip->chgic_mtk.oppo_info->ccdetect_active = pinctrl_lookup_state(chip->chgic_mtk.oppo_info->pinctrl, "ccdetect_active");
	if (IS_ERR_OR_NULL(chip->chgic_mtk.oppo_info->ccdetect_active)) {
		chg_err("get ccdetect_active fail\n");
		return -EINVAL;
	}

	chip->chgic_mtk.oppo_info->ccdetect_sleep = pinctrl_lookup_state(chip->chgic_mtk.oppo_info->pinctrl, "ccdetect_sleep");
	if (IS_ERR_OR_NULL(chip->chgic_mtk.oppo_info->ccdetect_sleep)) {
		chg_err("get ccdetect_sleep fail\n");
		return -EINVAL;
	}
	if (chip->chgic_mtk.oppo_info->ccdetect_gpio > 0) {
		gpio_direction_input(chip->chgic_mtk.oppo_info->ccdetect_gpio);
	}

	pinctrl_select_state(chip->chgic_mtk.oppo_info->pinctrl,  chip->chgic_mtk.oppo_info->ccdetect_active);
	return 0;
}

void oppo_ccdetect_work(struct work_struct *work)
{
	int level;
	struct oppo_chg_chip *chip = g_oppo_chip;
	oppo_get_usbtemp_volt(chip);
	get_usb_temp(chip);
	level = gpio_get_value(chip->chgic_mtk.oppo_info->ccdetect_gpio);
	if (level != 1) {
		oppo_ccdetect_enable();
		if(chip->usb_temp_l < 55 && chip->usb_temp_r < 55)
			usbtemp_callback(0);
        oppo_wake_up_usbtemp_thread();
	} else {
		if(chip->usb_temp_l < 55 && chip->usb_temp_r < 55)
...
void oppo_ccdetect_irq_init(struct oppo_chg_chip *chip)
{
	...
	chip->chgic_mtk.oppo_info->ccdetect_irq = gpio_to_irq(chip->chgic_mtk.oppo_info->ccdetect_gpio);
	printk(KERN_ERR "[OPPO_CHG][%s]: chip->chgic_mtk.oppo_info->ccdetect_gpio[%d]!\n", __func__, chip->chgic_mtk.oppo_info->ccdetect_gpio);
...
void oppo_ccdetect_irq_register(struct oppo_chg_chip *chip)
{
	int ret = 0;
	...
	ret = devm_request_threaded_irq(chip->dev,  chip->chgic_mtk.oppo_info->ccdetect_irq,
	...
	ret = enable_irq_wake(chip->chgic_mtk.oppo_info->ccdetect_irq);
	if (ret != 0) {
...
int oppo_chg_ccdetect_parse_dt(struct oppo_chg_chip *chip)
{
	...
	chip->chgic_mtk.oppo_info->ccdetect_gpio = of_get_named_gpio(node, "qcom,ccdetect-gpio", 0);
	if (chip->chgic_mtk.oppo_info->ccdetect_gpio <= 0) {
		chg_err("Couldn't read qcom,ccdetect-gpio rc=%d, qcom,ccdetect-gpio:%d\n",
				rc, chip->chgic_mtk.oppo_info->ccdetect_gpio);
	} else {
		if (oppo_ccdetect_support_check() == true) {
			rc = gpio_request(chip->chgic_mtk.oppo_info->ccdetect_gpio, "ccdetect-gpio");
			if (rc) {
				chg_err("unable to request ccdetect_gpio:%d\n",
						chip->chgic_mtk.oppo_info->ccdetect_gpio);
			} else {
...
static int oppo_chg_parse_custom_dt(struct oppo_chg_chip *chip)
{
	...
	rc = oppo_chg_ccdetect_parse_dt(chip);
	if (rc) {
		printk(KERN_ERR "[OPPO_CHG][%s]: oppo_chg_ccdetect_parse_dt fail!\n", __func__);
	}
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_USBTEMP_VOLT_L:
			if (g_oppo_chip)
				val->intval = g_oppo_chip->usb_temp_l;
			else
				val->intval = -ENODATA;
			break;
		case POWER_SUPPLY_PROP_USBTEMP_VOLT_R:
			if (g_oppo_chip)
				val->intval = g_oppo_chip->usb_temp_r;
			else
				val->intval = -ENODATA;
			break;
...
static enum power_supply_property mt_usb_properties[] = {
	...
	//POWER_SUPPLY_PROP_FLASH_SWITCH,
};
...
#endif
	chr_err("%s:oppo_battery_mtk6889 starts\n", __func__);
...
#endif
#ifdef VENDOR_EDIT
/* YanGang@BSP.CHG.Basic, 2019/12/24, sjc Add for ccdetect work */
	if (oppo_ccdetect_support_check() == true){
		INIT_DELAYED_WORK(&ccdetect_work,oppo_ccdetect_work);
