...
{
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		if(oppo_ccdetect_check_is_gpio(g_oppo_chip) == true){
			if(gpio_get_value(g_oppo_chip->chgic_mtk.oppo_info->ccdetect_gpio) == 0){
				printk(KERN_ERR "[OPPO_CHG][oppo_set_otg_switch_status]: gpio[L], should set, return\n");
				return;
			}
		}
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/11/09, sjc Add for charging*/
	...
	chr_err("%s: oppo_battery_mtk6885 starts\n", __func__);
