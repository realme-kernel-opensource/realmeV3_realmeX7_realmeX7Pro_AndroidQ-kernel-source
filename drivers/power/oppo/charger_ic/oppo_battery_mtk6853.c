...
/* LiYue@BSP.CHG.Basic, 2020/03/19, check sub current in engineer mode */
...
static int oppo_mt6360_charging_current_write_fast(int chg_curr)
{
	...
	if (g_oppo_chip->dual_charger_support &&
		...
		rc = bq2591x_charging_current_write_fast(slave_cur);
		if (rc < 0) {
...
aicl_pre_step:		
	...
	if (g_oppo_chip->dual_charger_support &&
		...
		chg_debug("usb input max current limit aicl: master and salve input current: %d, %d\n",
				usb_icl[i] * 1000 * (100-g_oppo_chip->slave_pct) / 100, usb_icl[i] * 1000 * g_oppo_chip->slave_pct / 100);
		bq2591x_input_current_limit_write(usb_icl[i] * 1000 * g_oppo_chip->slave_pct / 100);
		rc = charger_dev_set_input_current(chg, usb_icl[i] * 1000 * (100-g_oppo_chip->slave_pct) / 100);
	} else
		rc = charger_dev_set_input_current(chg, usb_icl[i] * 1000);
#else
	rc = charger_dev_set_input_current(chg, usb_icl[i] * 1000);
...
static int oppo_mt6360_float_voltage_write(int vfloat_mv)
{
	...
	rc = bq2591x_float_voltage_write(vfloat_mv + DELTA_MV);
	if (rc < 0) {
...
static int oppo_mt6360_pd_setup(void)
{
	...
	if (!chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90 && chip->temperature <= 400 && chip->cool_down_force_5v == false) {
		...
		if (chip->camera_on || chip->soc >= 90 || chip->batt_volt >= 4450 || chip->temperature > 400 || chip->cool_down_force_5v == true){
			vbus_mv = VBUS_5V;
			ibus_ma = IBUS_3A;
...
//====================================================================//
#ifdef CONFIG_OPPO_CHARGER_MATCH_MTKGAUGE
...
/* Jianchao.Shi@BSP.CHG.Basic, 2018/11/09, sjc Add for charging*/
	...
	chr_err("%s:oppo_battery_mtk6853 starts\n", __func__);
