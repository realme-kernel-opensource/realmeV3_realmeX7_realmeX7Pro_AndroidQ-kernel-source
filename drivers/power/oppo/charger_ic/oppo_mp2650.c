...
#endif
	chg_err("[OPPO_CHG][mp2650] chg_vol = %d\n", chg_vol);
    return chg_vol;
}
...
int mp2650_input_current_limit_ctrl_by_vooc_write(int current_ma)
{
	...
	for (count=(tmp / 500); count > 0; count--) {
		chg_err("[OPPO_CHG][mp2650]set charge current limit tmp = %d\n", 500*count);
	...
	for (count=1; count <= (current_ma /500); count++) {
		chg_err("[OPPO_CHG][mp2650]set charge current limit = %d\n", 500*count);
		tmp = 500*count - REG00_MP2650_1ST_CURRENT_LIMIT_OFFSET;
		tmp = tmp / REG00_MP2650_1ST_CURRENT_LIMIT_STEP;
		rc = mp2650_config_interface(REG00_MP2650_ADDRESS, tmp << REG00_MP2650_1ST_CURRENT_LIMIT_SHIFT, REG00_MP2650_1ST_CURRENT_LIMIT_MASK);
		rc = mp2650_config_interface(REG0F_MP2650_ADDRESS, tmp << REG0F_MP2650_2ND_CURRENT_LIMIT_SHIFT, REG0F_MP2650_2ND_CURRENT_LIMIT_MASK);
	...
	chg_err("[OPPO_CHG][mp2650]set charge ctrl_by_vooc current limit = %d\n", current_ma);
...
aicl_pre_step:
	chg_debug( "[OPPO_CHG][mp2650]usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_pre_step\n", chg_vol, i, mp2650_usbin_input_current_limit[i], sw_aicl_point);
	goto aicl_rerun;
aicl_end:
	chg_debug( "[OPPO_CHG][mp2650]usb input max current limit aicl chg_vol=%d j[%d]=%d sw_aicl_point:%d aicl_end\n", chg_vol, i, mp2650_usbin_input_current_limit[i], sw_aicl_point);
	goto aicl_rerun;
aicl_rerun:
...
{	
    ...
    chg_err("[OPPO_CHG][mp2650]set charge current = %d\n", chg_cur);
...
int mp2650_float_voltage_write(int vfloat_mv)
{
    ...
    chg_err("[OPPO_CHG][mp2650] vfloat_mv = %d\n", vfloat_mv); 
...
int mp2650_set_termchg_current(int term_curr)
{
    ...
    chg_err("[OPPO_CHG][mp2650] term_current = %d\n", term_curr); 
...
int mp2650_enable_charging(void)
{
	...
	chg_err("[OPPO_CHG][mp2650] mp2650_enable_charging \n");
	return rc;
}
...
int mp2650_disable_charging(void)
{
    ...
    chg_err("[OPPO_CHG][mp2650] mp2650_disable_charging \n");
...
int mp2650_get_ibus_current(void)
{
    ...
    chg_err("[OPPO_CHG][mp2650]cur_high = 0x%x, cur_low = 0x%x, ibus_curr[%d]\n", cur_high, cur_low, ibus_curr);
...
int mp2650_registers_read_full(void)
{
	...
	if (reg_full) {
		chg_err("[OPPO_CHG][mp2650]the mp2650 is full");
...
int mp2650_enable_hiz(void)
{
	...
	chg_err("[OPPO_CHG][mp2650]mp2650_enable_hiz \n");
...
int mp2650_disable_hiz(void)
{
	...
	chg_err("[OPPO_CHG][mp2650]mp2650_disable_hiz \n");
...
int mp2650_check_learn_mode(void)
{
	...
	chg_err("[OPPO_CHG][mp2650] LEARN MODE = %02x\n", reg_val);
...
void mp2650_dump_registers(void)
{
    ...
    if(dump_count == DUMP_REG_LOG_CNT_30S) {
        ...
        printk(KERN_ERR "mp2650[1A-22, 48, 49  2D]: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, reg48=0x%x, reg49=0x%x, reg0x2d=0x%x \n", 
...
int mp2650_hardware_init(void)
{
    ...
    chg_err("[OPPO_CHG][mp2650] init mp2650 hardware for wireless charge! \n");
...
static int rtc_reset_check(void)
{
    ...
    chg_debug("[OPPO_CHG][mp2650] : Sec: %d, Min: %d, Hour: %d, Day: %d, Mon: %d, Year: %d  ###  wday: %d, yday: %d, isdst: %d\n",
    tm.tm_sec, tm.tm_min, tm.tm_hour, tm.tm_mday, tm.tm_mon, tm.tm_year,
    tm.tm_wday, tm.tm_yday, tm.tm_isdst);
...
{
	chg_err("[OPPO_CHG][mp2650]  oppo_get_chg_ops--mp2650_chg_ops\n");
...
static int mp2650_mps_otg_en_gpio_init(struct chip_mp2650 *chip)
{
	...
	pinctrl_select_state(chip->pinctrl, chip->mps_otg_en_sleep);
  	chg_err("[OPPO_CHG][mp2650] tongfeng test, gpio_val:%d\n", gpio_get_value(chip->mps_otg_en_gpio));
...
static int mp2650_gpio_init(struct chip_mp2650 *chip)
{
    ...
    chg_err("[OPPO_CHG][mp2650]  mp2650_gpio_init FINISH\n");
...
{    
	...
	pr_err("%s: end \n", __func__);
	chg_err("[OPPO_CHG][mp2650] set value:%d, gpio_val:%d\n", 
...
static ssize_t mp2650_data_log_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
    ...
    pr_err("[OPPO_CHG][mp2650]  %s: input data = %s,  write_mp2650_data = 0x%02X\n", __func__, write_data, critical_log);
...
static ssize_t mp2650_data_log_read(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
    ...
    pr_err("[OPPO_CHG][mp2650]  %s: input data = %s,  mp2650_addr = 0x%02X\n", __func__, write_data, mp2650_add);
...
static int mp2650_driver_probe(struct i2c_client *client, const struct i2c_device_id *id) 
{
    ...
    chg_debug( "[OPPO_CHG][mp2650]  call \n");
    ...
    chg_debug("[OPPO_CHG][mp2650]  success\n");
