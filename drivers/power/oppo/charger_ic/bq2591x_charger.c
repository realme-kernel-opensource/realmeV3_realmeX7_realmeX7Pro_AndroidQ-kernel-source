...
/*laibaoquan*/
static struct bq2591x *bq_chargeric = NULL;
/*end*/
...
#ifndef CONFIG_OPPO_DUAL_CHARGER_SUPPORT
...
/*laibaoquan*/
#ifdef CONFIG_OPPO_DUAL_CHARGER_SUPPORT

void bq2591x_dump_registers(void)    
{
	struct bq2591x *bq = bq_chargeric;
...
int bq2591x_hardware_init(void)      
{
	int ret;
	struct bq2591x *bq = bq_chargeric;
...
int bq2591x_charging_current_write_fast(int current_ma)
{
	int ret = 0;
	struct bq2591x *bq = bq_chargeric;
	
	if (NULL == bq_chargeric)
		return -1;
	
	ret = bq2591x_set_fast_chg_current(bq_chargeric, current_ma);
	
	return ret;
}

int bq2591x_input_current_limit_write(int curr)
{
	int ret = 0;
	struct bq2591x *bq = bq_chargeric;
	...
	ret = bq2591x_set_input_current_limit(bq_chargeric,curr);
	
	return ret;

}

int bq2591x_float_voltage_write(int volt)
{
	int ret = 0;
	struct bq2591x *bq = bq_chargeric;
	...
	ret = bq2591x_set_chargevoltage(bq_chargeric,volt);
	
	return ret;

}

int bq2591x_enable_charging(void)                 
{
	int rc;
	struct bq2591x *bq = bq_chargeric;
	u8 val = BQ2591X_CHG_ENABLE << BQ2591X_EN_CHG_SHIFT;

	rc = bq2591x_update_bits(bq, BQ2591X_REG_06,
				BQ2591X_EN_CHG_MASK, val);
				
	if (rc < 0) {
		chg_err("Couldn't bq2591x_enable_charging rc = %d\n", rc);
	}
...
int bq2591x_disable_charging(void)
{
	int rc;
	struct bq2591x *bq = bq_chargeric;
	u8 val = BQ2591X_CHG_DISABLE << BQ2591X_EN_CHG_SHIFT;
	...
	if (rc < 0) {
		chg_err("Couldn't bq2591x_disable_charging rc = %d\n", rc);
	}
...
/*struct oppo_chg_operations  bq2591x_chg_ops = {
	.dump_registers = bq2591x_dump_registers,
	.hardware_init = bq2591x_hardware_init,
	.charging_current_write_fast = bq2591x_charging_current_write_fast,
	.input_current_write = bq2591x_input_current_limit_write,
	.charging_enable = bq2591x_enable_charging,
	.charging_disable = bq2591x_disable_charging,
	.float_voltage_write = bq2591x_float_voltage_write,
};*/
...
MODULE_DESCRIPTION("TI BQ2591x Charger Driver laibaoquan");
MODULE_LICENSE("GPL2");
