...
static ssize_t da9313_reg_address_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    uint8_t ret = 0;
    char page[24];
    struct chip_da9313 *divider_ic = the_chip;
    ...
    sprintf(page, "0x%x\n", divider_ic->reg_address);
    ret = simple_read_from_buffer(buf, count, ppos, page, strlen(page));

    return ret;
}

static ssize_t da9313_reg_address_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
    char buffer[24] = {0};
    struct chip_da9313 *divider_ic = the_chip;
    ...
    if (copy_from_user(buffer, buf, 20)) {
        chg_err("%s: read proc input error.\n", __func__);
        return count;
    }

    sscanf(buffer, "0x%x", &(divider_ic->reg_address));

    return count;
}


static const struct file_operations da9313_reg_address_ops =
{
    .read = da9313_reg_address_read,
    .write  = da9313_reg_address_write,
    .open  = simple_open,
    .owner = THIS_MODULE,
};

static ssize_t da9313_reg_data_read(struct file *file, char __user *buf, size_t count, loff_t *ppos)
{
    uint8_t ret = 0;
    char page[24];
    int val = 0;
    ...
    ret = da9313_read_reg(divider_ic->reg_address, &val);
...
static ssize_t da9313_reg_data_write(struct file *file, const char __user *buf, size_t count, loff_t *lo)
{
    char buffer[24] = {0};
    int val = 0;
    struct chip_da9313 *divider_ic = the_chip;
    ...
    //chg_err("ymj: buf: %s val: 0x%x\n", buffer, val);
    da9313_config_interface(REG0D_DA9313_ADDRESS, DA9313_UNLOCK_VAL, REG0D_DA9313_LOCK_REG_MASK);//unlock first
    msleep(200);

    da9313_config_interface(divider_ic->reg_address, val, 0xff);

    da9313_config_interface(REG0D_DA9313_ADDRESS, 0, REG0D_DA9313_LOCK_REG_MASK);//lock again

    return count;
}


static const struct file_operations da9313_reg_data_ops =
{
    .read = da9313_reg_data_read,
    .write  = da9313_reg_data_write,
    .open  = simple_open,
    .owner = THIS_MODULE,
};
...
static int init_da9313_proc(struct chip_da9313 *da)
{
    ...
    prEntry_tmp = proc_create_data("data", 0644, prEntry_da, &da9313_reg_data_ops, da);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        chg_debug("%s: Couldn't create da9313_reg_data_ops entry, %d\n", __func__, __LINE__);
    }
    prEntry_tmp = proc_create_data("address", 0644, prEntry_da, &da9313_reg_address_ops, da);
    if (prEntry_tmp == NULL) {
        ret = -ENOMEM;
        chg_debug("%s: Couldn't create da9313_reg_address_ops entry, %d\n", __func__, __LINE__);
    }

    return 0;
}
static bool da9313_parse_dt(struct chip_da9313 *chip)
{
	struct device_node *node = chip->dev->of_node;
	bool ic_en_mode = 0;
	ic_en_mode = of_property_read_bool(node, "qcom,ic_en_mode");
	return ic_en_mode;
}
...
{             
	...
	if(da9313_parse_dt(divider_ic)){
		//da9313_config_interface(REG02_DA9313_ADDRESS, REG02_DA9313_ICEN_MODE_EN, REG02_DA9313_ICEN_MODE_MASK);
		chg_err("%s: da9313 reset and enter ICEN_MODE\n", __func__);
	}
	 if (get_project() == 20630 ||get_project() == 20631||get_project() == 20632 ||get_project() == 20633 ||get_project() == 20634 ||get_project() == 20635){
		chg_err("%s: da9313 ICEN_MODE,get_PCB_Version:%d\n", __func__,get_PCB_Version());
		if(get_PCB_Version() >= 5){
			chg_err("%s: da9313 set ICEN_MODE \n", __func__);
			//da9313_config_interface(REG02_DA9313_ADDRESS, REG02_DA9313_ICEN_MODE_EN, REG02_DA9313_ICEN_MODE_MASK);
		}
	 }
    return 0;
