...
static int cpufreq_set_cpu_scaling_limit(u32 c_index, unsigned int cpu, unsigned int min,
        unsigned int max)
{
	struct cpufreq_policy *policy;
	struct hypnus_data *hypdata = hypnus_get_hypdata();
	unsigned int adj_max, adj_min;

	adj_max = mt_cpufreq_find_close_freq(c_index, max);
	adj_min = mt_cpufreq_find_close_freq(c_index, min);

	if (!cpu_online(cpu))
		return 0;

	if (hypdata->forceflag == THERM_FORCE_FLAG) {
		hypdata->ceiling_freq[c_index] = adj_max;
		hypdata->floor_freq[c_index] = adj_min;
	}
	...
	/*
	 *(1) freq not in table will cause deadloop in mtk function get_freq_capacity()
	 *(2) cpufreq_driver_resolve_freq() can't fix this issue, because target_index
	 * and resolve_freq are not implemented in mediatek cpufreq driver.
	 *(3) cpufreq_frequency_table_target() also can't fix this issue because it will
	 * limit target_freq between policy->min and policy->max, but we are trying to update
	 * cpufreq policy here.
	 * Now add a check here to avoid frequency not in table.
	 * TODO: we need to find proper frequency int the further.
	 */
	if (cpufreq_frequency_table_get_index(policy, adj_min) < 0 ||
			cpufreq_frequency_table_get_index(policy, adj_max) < 0) {
		pr_info("CPU %d %lu %lu, Frequency not in table\n", cpu, adj_min, adj_max);
		return -EINVAL;
	}
	policy->user_policy.min = adj_min;
	policy->user_policy.max = adj_max;
...
static int mt_set_cpu_freq_limit(u32 c_index, u32 min, u32 max)
{
	...
	if (cpufreq_set_cpu_scaling_limit(c_index, cpu, min, max) < 0) {
...
static int mt_set_mdla_freq_limit(u32 core_id, u32 min, u32 max)
{
	...
	return 0;//mdla_lock_set_power(&mdla_lock_power);
}
...
static struct hypnus_chipset_operations mediatek_op = {
	...
	.set_cpunr_limit = mt_set_cpunr_limit,
