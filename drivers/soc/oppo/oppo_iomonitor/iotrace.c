...
static struct io_trace_action all_trace_action[] =
{
	...
	{SYS_READ_TIMEOUT_TAG, "sys_read_timeout"},
	{SYS_WRITE_TIMEOUT_TAG, "sys_write_timeout"},
	{SYS_SYNC_TIMEOUT_TAG, "sys_sync_timeout"},

	{BLOCK_GETRQ_TAG, "block_getrq"},
	{BLOCK_RQ_INSERT_TAG, "block_rq_insert"},
	{BLOCK_RQ_ISSUE_TAG, "block_rq_issue"},
	{BLOCK_RQ_COMPLETE_TAG, "block_rq_complete"},
	{BLOCK_RQ_REQUEUE_TAG, "block_rq_requeue"},
	{BLOCK_RQ_SLEEP_TAG, "block_sleeprq"},

	{SCSI_BLK_CMD_RW_ERROR_TAG, "scsi_dispatch_cmd_error"},
	{SCSI_BLK_CMD_RW_TIMEOUT_TAG, "scsi_dispatch_cmd_timeout"},
	{SCHED_STAT_IOWAIT_TAG, "sched_stat_iowait"},
...
static inline int is_syscall_action(unsigned int action)
{
	...
	if(action >= TRACE_BEGIN_TAG &&
	   action <= BLOCK_BEGIN_TAG)
	{
		return 1;
	}
...
{
	/*only the device log*/
	if(action > BLOCK_END_TAG &&
	   action <= TRACE_END_TAG)
	{
		return 1;
	}
...
static inline struct io_trace_entry_head *get_entry_head(int pos)
{
	...
	return (struct io_trace_entry_head *)(io_trace_this->all_trace_buff + pos);
}
...
static inline void io_trace_record_log(char type, char entry_size, void *entry)
{
	...
	{
		if(io_trace_this->first == 0 && io_trace_this->last == io_trace_this->size)
		{
			first =1 ;
...
static void io_trace_global_log(unsigned int action, unsigned long sector, unsigned int nr_bytes, struct file *filp,
                                unsigned int rw_flags, struct request_queue *rq,unsigned int dev_major,unsigned int dev_minor,
                                int result,struct task_struct *tsk,u64 delay)
{
	...
	if(is_block_action(action))
	{
		...
		blk_entry.rw = (unsigned char)rw_flags;
		...
		{
			blk_entry.in_flight_sync = (unsigned short)rq->in_flight[1];
			blk_entry.in_flight_async = (unsigned short)rq->in_flight[0];

		}
		blk_entry.time = log_time;

		io_trace_record_log(TRACE_BLK, sizeof(struct io_trace_blk_entry), &blk_entry);
	}
	else if(is_dev_action(action))
	{
		dev_entry.action = action;
		dev_entry.rw = (unsigned char)rw_flags;
		dev_entry.sector = sector;
		dev_entry.result = result;
		dev_entry.time = log_time;

		io_trace_record_log(TRACE_DEV, sizeof(struct io_trace_dev_entry), &dev_entry);
	}
	else if(action == SCHED_STAT_IOWAIT_TAG)
	{
		...
		snprintf(iowait_entry.wchan,sizeof(iowait_entry.wchan),"%ps",(void*)get_wchan(tsk));

		io_trace_record_log(TRACE_IOWAIT, sizeof(struct io_trace_iowait_entry), &iowait_entry);
	}
	else if(is_syscall_action(action))
	{
		...
		memcpy(syscall_entry.f_name, filp->f_path.dentry->d_iname, IO_F_NAME_LEN);

		syscall_entry.time = log_time;

		io_trace_record_log(TRACE_SYSCALL, sizeof(struct io_trace_syscall_entry), &syscall_entry);
	}
	return;
}
...
int write_log_kernel(char **data)
{
	...
	while(pos >= io_trace_this->first
	      && pos < io_trace_this->last)
	{
		...
		{
			case TRACE_SYSCALL:
				syscall_entry = (struct io_trace_syscall_entry *)entry_head->val;
				len = snprintf(buf,sizeof(buf),"%lu %d %d %s %s %llu\n",syscall_entry->time,syscall_entry->action,
				               syscall_entry->pid,syscall_entry->comm,syscall_entry->f_name,syscall_entry->delay);
				ret = kernel_write(filp, buf, len, &file_pos);
				break;
			case TRACE_IOWAIT:
				iowait_entry = (struct io_trace_iowait_entry *)entry_head->val;
				len = snprintf(buf,sizeof(buf),"%lu %d %d %s %s %llu %d\n",iowait_entry->time,iowait_entry->action,
				               iowait_entry->pid,iowait_entry->comm,iowait_entry->wchan,iowait_entry->delay,iowait_entry->ux_flag);
				ret = kernel_write(filp, buf, len, &file_pos);
				break;
			case TRACE_BLK:
				blk_entry = (struct io_trace_blk_entry *)entry_head->val;
				if(blk_entry->action == BLOCK_GETRQ_TAG)
					len = snprintf(buf,sizeof(buf),"%lu %d %d %d %d %lu %d %u %u\n",blk_entry->time,blk_entry->action,
					               blk_entry->dev_major,blk_entry->dev_minor,(unsigned int)(blk_entry->rw),blk_entry->sector,
					               blk_entry->nr_bytes,blk_entry->nr_rqs_sync,blk_entry->nr_rqs_async);
				else if(blk_entry->action == BLOCK_RQ_ISSUE_TAG)
					len = snprintf(buf,sizeof(buf),"%lu %d %d %d %d %lu %d %u %u\n",blk_entry->time,blk_entry->action,
					               blk_entry->dev_major,blk_entry->dev_minor,(unsigned int)(blk_entry->rw),blk_entry->sector,
					               blk_entry->nr_bytes,blk_entry->in_flight_sync,blk_entry->in_flight_async);
				else
					len = snprintf(buf,sizeof(buf),"%lu %d %d %d %d %lu %d\n",blk_entry->time,blk_entry->action,
					               blk_entry->dev_major,blk_entry->dev_minor,(unsigned int)(blk_entry->rw),blk_entry->sector,
					               blk_entry->nr_bytes);
				ret = kernel_write(filp, buf, len, &file_pos);
				break;
			case TRACE_DEV:
				dev_entry = (struct io_trace_dev_entry *)entry_head->val;
				len = snprintf(buf,sizeof(buf),"%lu %d %d %lu %d\n",dev_entry->time,dev_entry->action,
				               (unsigned int)(dev_entry->rw),dev_entry->sector,dev_entry->result);
				ret = kernel_write(filp, buf, len, &file_pos);
				break;
			case TRACE_END:
				pos = io_trace_this->size;
...
static void io_syscall_read_timeout(void *ignore, struct file *file, u64 delay)
{
	...
	if(io_trace_this->enable)
	{
		io_trace_global_log(SYS_READ_TIMEOUT_TAG, 0, 0, file, 0,NULL,0,0,-1000,NULL,delay);
	}
}
...
static void io_syscall_write_timeout(void *ignore, struct file *file, u64 delay)
{
	...
	if(io_trace_this->enable)
	{
		io_trace_global_log(SYS_WRITE_TIMEOUT_TAG, 0, 0, file, 0,NULL,0,0,-1000,NULL,delay);
	}
}
...
static void io_syscall_sync_timeout(void *ignore, struct file *file, u64 delay)
{
	...
	if(io_trace_this->enable)
	{
		io_trace_global_log(SYS_SYNC_TIMEOUT_TAG, 0, 0, file, 0,NULL,0,0,-1000,NULL,delay);
	}
}

static void blk_add_trace_rq_get(void *ignore, struct request_queue *q, struct bio *bio, int rw)
{
	if(io_trace_this->enable && bio)
	{
		unsigned int rw_type = bio->bi_opf;
		unsigned int dev = bio_dev(bio);
		io_trace_global_log(BLOCK_GETRQ_TAG,  bio->bi_iter.bi_sector, bio->bi_iter.bi_size,
		                    0, rw_type, q,MAJOR(dev),MINOR(dev),-1000,NULL,0);
	}
...
static void blk_add_trace_rq_insert(void *ignore, struct request_queue *q, struct request *rq)
{
	if(io_trace_this->enable)
	{
		...
		io_trace_global_log(BLOCK_RQ_INSERT_TAG, blk_rq_trace_sector(rq), blk_rq_bytes(rq),
		                    0, rw_type, q,MAJOR(dev),MINOR(dev),-1000,NULL,0);
	}
...
static void blk_add_trace_rq_issue(void *ignore, struct request_queue *q, struct request *rq)
{
	...
	if(io_trace_this->enable)
	{
		unsigned int rw_type = rq->cmd_flags;
		unsigned int dev = rq->rq_disk ? disk_devt(rq->rq_disk) : 0;
		io_trace_global_log(BLOCK_RQ_ISSUE_TAG, blk_rq_trace_sector(rq), blk_rq_bytes(rq),
		                    0, rw_type, q,MAJOR(dev),MINOR(dev),-1000,NULL,0);
	}
                                      ...
                                      int error, unsigned int nr_bytes)
{
	if(io_trace_this->enable)
	{
		unsigned int rw_type = rq->cmd_flags;
		unsigned int dev = rq->rq_disk ? disk_devt(rq->rq_disk) : 0;
		io_trace_global_log(BLOCK_RQ_COMPLETE_TAG, blk_rq_trace_sector(rq), blk_rq_bytes(rq),
		                    0, rw_type, NULL,MAJOR(dev),MINOR(dev),-1000,NULL,0);
	}
	return;
}
...
static void blk_add_trace_rq_requeue(void *ignore, struct request_queue *q, struct request *rq)
{
	...
	if(io_trace_this->enable)
	{
		unsigned int rw_type = rq->cmd_flags;
		unsigned int dev = rq->rq_disk ? disk_devt(rq->rq_disk) : 0;
		io_trace_global_log(BLOCK_RQ_REQUEUE_TAG, blk_rq_trace_sector(rq), blk_rq_bytes(rq),
		                    0, rw_type, q,MAJOR(dev),MINOR(dev),-1000,NULL,0);
	}
...
static void scsi_dispatch_cmd_timeout(void *ignore, struct scsi_cmnd *cmd)
{
	...
	if (io_trace_this->enable)
	{
		io_trace_global_log(SCSI_BLK_CMD_RW_TIMEOUT_TAG, (*addr) * 8, (*size) * 16, 0, rw_flags,NULL,0,0,result,NULL,0);
	}
	return;
}

static void scsi_dispatch_cmd_error(void *ignore, struct scsi_cmnd *cmd, int rtn)
{
	unsigned int rw_flags = 0;
	unsigned char c[4];
	unsigned char len[2];
	...
	if (io_trace_this->enable)
	{
		io_trace_global_log(SCSI_BLK_CMD_RW_ERROR_TAG, (*addr) * 8, (*size) * 16, 0, rw_flags,NULL,0,0,result,NULL,0);
	}
	return;
}
...
static void sched_stat_iowait(void *ignore, struct task_struct *tsk, u64 delay)
{
	...
	{
/*
		if(test_task_ux(tsk) && io_ns_to_ms(delay) >= UX_IO_WAIT_TIMEOUT)
			io_trace_global_log(SCHED_STAT_IOWAIT_TAG, 0, 0, 0, 0,NULL,0,0,1,tsk,delay);
		else
*/
		if(io_ns_to_ms(delay) >= IO_WAIT_TIMEOUT)
			io_trace_global_log(SCHED_STAT_IOWAIT_TAG, 0, 0, 0, 0,NULL,0,0,0,tsk,delay);
	}
...
static void iotrace_register_tracepoints(void)
{
	...
	/*syscall*/
	ret = register_trace_syscall_read_timeout(io_syscall_read_timeout, NULL);
	WARN_ON(ret);
	ret = register_trace_syscall_write_timeout(io_syscall_write_timeout, NULL);
	WARN_ON(ret);
	ret = register_trace_syscall_sync_timeout(io_syscall_sync_timeout, NULL);
	WARN_ON(ret);
	...
	ret = register_trace_scsi_dispatch_cmd_error(scsi_dispatch_cmd_error, NULL);
	WARN_ON(ret);
	ret = register_trace_scsi_dispatch_cmd_timeout(scsi_dispatch_cmd_timeout, NULL);
	WARN_ON(ret);
