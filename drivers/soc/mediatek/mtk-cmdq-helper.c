...
#ifdef VENDOR_EDIT
/* LiPing-M@PSW.MM.DisplayDriver.Stability, 2019/12/05, bugid:2621310 add for dc backlight */
/* sleep for 1300 tick, which around 50us */
extern bool oppo_dc_set;
#endif

/* sleep for 320 tick, which around 12us */
#define CMDQ_POLL_TICK			320
...
struct cmdq_client *cmdq_mbox_create(struct device *dev, int index)
{
	...
	if (IS_ERR(client->chan)) {
		cmdq_err("channel request fail:%d", PTR_ERR(client->chan));
		kfree(client);
		return NULL;
	}
