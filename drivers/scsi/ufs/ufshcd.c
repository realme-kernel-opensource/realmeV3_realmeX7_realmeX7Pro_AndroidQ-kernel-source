...
#ifdef VENDOR_EDIT
/* Lingfei.Tang@BSP.BSP.Power, 2019/11/26, Update mediatek patch for ufs power */
static inline void ufshcd_clkgate_cancel_gate_work(struct ufs_hba *hba)
{
	hrtimer_cancel(&hba->clk_gating.gate_timer);
...
static void ufshcd_ungate_work(struct work_struct *work)
{
	...
	#ifdef VENDOR_EDIT
	/* Lingfei.Tang@BSP.BSP.Power, 2019/11/26, Update mediatek patch for ufs power */
	ufshcd_clkgate_cancel_gate_work(hba);
	#else
...
start:
	...
	case REQ_CLKS_OFF:
		#ifdef VENDOR_EDIT
		/* Lingfei.Tang@BSP.BSP.Power, 2019/11/26, Update mediatek patch for ufs power */
		if (hrtimer_try_to_cancel(&hba->clk_gating.gate_timer) == 1) {
		#else
...
static void __ufshcd_release(struct ufs_hba *hba)
{
	...
	#ifdef VENDOR_EDIT
	/* Lingfei.Tang@BSP.BSP.Power, 2019/11/26, Update mediatek patch for ufs power */
	hrtimer_start(&hba->clk_gating.gate_timer,
...
static void ufshcd_init_clk_gating(struct ufs_hba *hba)
{
	...
	#ifdef VENDOR_EDIT
	/* Ji.Xu@BSP.BSP.Power, 2019/11/21,  Add temp patch for power debug*/
	hba->clk_gating.delay_ms = 10;
	/* Lingfei.Tang@BSP.BSP.Power, 2019/11/26, Update mediatek patch for ufs power */
	INIT_WORK(&hba->clk_gating.gate_work, ufshcd_gate_work);
	INIT_WORK(&hba->clk_gating.ungate_work, ufshcd_ungate_work);

	hrtimer_init(&hba->clk_gating.gate_timer, CLOCK_MONOTONIC,
			     HRTIMER_MODE_REL);
	hba->clk_gating.gate_timer.function = ufshcd_clkgate_hrtimer_handler;
