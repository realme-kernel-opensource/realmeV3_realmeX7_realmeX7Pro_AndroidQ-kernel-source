...
#define SPI_MODE_MASK                                                         \
  (SPI_CPHA | SPI_CPOL | SPI_CS_HIGH | SPI_LSB_FIRST | SPI_3WIRE | SPI_LOOP | \
   SPI_NO_CS | SPI_READY | SPI_TX_DUAL | SPI_TX_QUAD | SPI_RX_DUAL |          \
   SPI_RX_QUAD)
...
static ssize_t st54spi_sync(struct st54spi_data *st54spi,
  ...
  if (status == 0) status = message->actual_length;
...
static ssize_t st54spi_read(struct file *filp, char __user *buf, size_t count,
  ...
  if (count > bufsiz) return -EMSGSIZE;
...
static int st54spi_message(struct st54spi_data *st54spi,
  ...
  for (n = n_xfers, k_tmp = k_xfers, u_tmp = u_xfers; n;
    ...
    if (!k_tmp->speed_hz) k_tmp->speed_hz = st54spi->speed_hz;
#ifdef VERBOSE
...
static struct spi_ioc_transfer *st54spi_get_ioc_message(
    unsigned int cmd, struct spi_ioc_transfer __user *u_ioc,
    unsigned int *n_ioc) {
  ...
  tmp = _IOC_SIZE(cmd);
  if ((tmp % sizeof(struct spi_ioc_transfer)) != 0) return ERR_PTR(-EINVAL);
  *n_ioc = tmp / sizeof(struct spi_ioc_transfer);
  if (*n_ioc == 0) return NULL;

  /* copy into scratch area */
  ioc = kmalloc(tmp, GFP_KERNEL);
  if (!ioc) return ERR_PTR(-ENOMEM);
...
static void st54spi_power_off(struct st54spi_data *st54spi) {
  if (st54spi->power_gpio_mode == POWER_MODE_ST54H) {
#ifdef ST21NFCD_MTK
    // Change NSS polarity to have NSS low.
    if (st54spi->spi && st54spi->spi->controller_data) {
      struct mtk_chip_config *orig_chip_config = st54spi->spi->controller_data;
...
#else
      pr_err("%s : change NSS polarity to be implemented!\n", __func__);
#endif
    // Set SE_PWR_REQ to low
    if (st54spi->power_gpio) gpio_set_value(st54spi->power_gpio, 0);
  ...
  } else if (st54spi->power_gpio_mode == POWER_MODE_ST54J) {
  ...
  pr_info("%s : disabling PMU clock of SPI subsystem\n", __func__);
...
static void st54spi_power_on(struct st54spi_data *st54spi) {
  ...
  pr_info("%s : enabling PMU clock of SPI subsystem\n", __func__);
  ...
  } else if (st54spi->power_gpio_mode == POWER_MODE_ST54H) {
    // set SE_PWR_REQ
    if (st54spi->power_gpio) gpio_set_value(st54spi->power_gpio, 1);
...
static void st54spi_power_set(struct st54spi_data *st54spi, int val) {
  ...
  if (debug_enabled) pr_info("st54spi sehal pwr_req: %d\n", val);

  if (val) {
    st54spi->sehal_needs_poweron = 1;
    if (st54spi->se_is_poweron == 0) st54spi_power_on(st54spi);
  } else {
    st54spi->sehal_needs_poweron = 0;
    if ((st54spi->se_is_poweron == 1) && (st54spi->nfcc_needs_poweron == 0))
...
static int st54spi_power_get(struct st54spi_data *st54spi) {
  if (st54spi->power_gpio) return gpio_get_value(st54spi->power_gpio);
  return 0;
}
...
static long st54spi_ioctl(struct file *filp, unsigned int cmd,
  ...
  switch (cmd) {
    ...
    case SPI_IOC_RD_MODE32:
      retval = __put_user(spi->mode & SPI_MODE_MASK, (__u32 __user *)arg);
      break;
    case SPI_IOC_RD_LSB_FIRST:
      retval =
          __put_user((spi->mode & SPI_LSB_FIRST) ? 1 : 0, (__u8 __user *)arg);
      break;
    ...
    default:
      ...
      ioc = st54spi_get_ioc_message(cmd, (struct spi_ioc_transfer __user *)arg,
                                    &n_ioc);
      if (IS_ERR(ioc)) {
        retval = PTR_ERR(ioc);
        break;
...
static long st54spi_compat_ioc_message(struct file *filp, unsigned int cmd,
  ...
  if (!access_ok(VERIFY_READ, u_ioc, _IOC_SIZE(cmd))) return -EFAULT;
...
static int st54spi_open(struct inode *inode, struct file *filp) {
  ...
  if (debug_enabled) pr_info("st54spi: open - force power on\n");
...
static int st54spi_release(struct inode *inode, struct file *filp) {
  ...
  if (!st54spi->users) {
    ...
    if (debug_enabled) pr_info("st54spi: release - may allow power off\n");
    ...
    spin_lock_irq(&st54spi->spi_lock);
    if (st54spi->spi) st54spi->speed_hz = st54spi->spi->max_speed_hz;

    /* ... after we unbound from the underlying device? */
    dofree = ((st54spi->spi == NULL) && (st54spi->spi_reset == NULL));
    spin_unlock_irq(&st54spi->spi_lock);

    if (dofree) kfree(st54spi);
  }
  mutex_unlock(&device_list_lock);
...
static void st54spi_probe_acpi(struct spi_device *spi) {
  ...
  if (!has_acpi_companion(&spi->dev)) return;

  id = acpi_match_device(st54spi_acpi_ids, &spi->dev);
  if (WARN_ON(!id)) return;
}
#else
...
static void st54spi_st21nfc_cb(int dir, void *data) {
  ...
  switch (dir) {
    case ST54SPI_CB_RESET_START:
      if (st54spi->se_is_poweron) st54spi_power_off(st54spi);
      break;
    ...
    case ST54SPI_CB_ESE_NOT_USED:
      ...
      if ((st54spi->se_is_poweron == 1) && (st54spi->sehal_needs_poweron == 0))
        // we don t need power anymore
        st54spi_power_off(st54spi);
      break;
  }
...
static int st54spi_probe(struct spi_device *spi) {
...
#else
      pr_err("%s : TSU_NSS configuration be implemented!\n", __func__);
  ...
  if (st54spi->power_gpio != 0) {
    int default_value = 0;
    if (st54spi->power_gpio_mode == POWER_MODE_ST54J_COMBO) default_value = 1;
    ...
    if (ret) pr_info("%s : power request failed (%d)\n", __FILE__, ret);

    pr_info("%s : power GPIO = %d\n", __func__, st54spi->power_gpio);
    ret = gpio_direction_output(st54spi->power_gpio, default_value);
    if (ret) pr_info("%s : reset direction_output failed\n", __FILE__);

    /* active high */
    gpio_set_value(st54spi->power_gpio, default_value);
  }
...
#else
    pr_err("%s : st54spi as module cannot use ST54H fully\n", __func__);
#endif
  }
...
static int st54spi_remove(struct spi_device *spi) {
  ...
  if (st54spi->users == 0) kfree(st54spi);
  mutex_unlock(&device_list_lock);
...
static int __init st54spi_init(void) {
  ...
  spidev_major = __register_chrdev(0, 0, N_SPI_MINORS, "spi", &st54spi_fops);
