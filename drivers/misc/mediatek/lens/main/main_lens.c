...
static struct stAF_DrvList g_stAF_DrvList[MAX_NUM_OF_LENS] = {
	...
	/* WenJun@Cam.Drv, 20200413, add for 20001 AF*/
	...
	{1, AFDRV_BU64253GWZAF, BU64253GWZAF_SetI2Cclient, BU64253GWZAF_Ioctl,
	 BU64253GWZAF_Release, BU64253GWZAF_GetFileName, NULL, NULL},
	/*{1, AFDRV_DW9839AF, DW9839AF_SetI2Cclient, DW9839AF_Ioctl,
	 DW9839AF_Release, DW9839AF_GetFileName, NULL, NULL},*/
	{1, AFDRV_BU63169AF, BU63169AF_SetI2Cclient, BU63169AF_Ioctl,
	 BU63169AF_Release, BU63169AF_GetFileName, NULL, BU63169AF_OisGetHallInfo},
...
#define SENSOR_IDX_MAIN 0
static struct regulator *regVCAMAF = NULL;
static int g_regVCAMAFEn;
extern struct regulator *regulator_get_regVCAMAF(void);
extern struct regulator *regulator_get_regVCAMAF_19040(int sensor_idx);
/* Shipei.Chen@Cam.Drv, 20200422, Add for AF power on and off of 20601 */
#define MAIN_AF_LDO_NUM (6)
#define MAIN_AF_VOLTAGE (1800)
/* Shipei.Chen@Cam.Drv, 20200526,  af porting for 20630!*/
#define ATHENSB_MAIN_AF_LDO_NUM (5)
#define ATHENSB_MAIN_AF_VOLTAGE (2800)


extern int Cam_LDO_PowerOn_Fan53870(int ldo_num, int set_mv_avdd);
extern int Cam_LDO_PowerDown_Fan53870(int ldo_num);
...
void AFRegulatorCtrl(int Stage)
{
	...
	if (Stage == 0) {
		if (regVCAMAF == NULL) {
			...
			#else
			//wenhui.chen@Cam.Drv, 20200402, add for  20601 lens
			if (is_project(OPPO_19040)) {
				regVCAMAF = regulator_get_regVCAMAF_19040(SENSOR_IDX_MAIN);
			} else if (is_project(OPPO_20601) || is_project(OPPO_20660) ||is_project(OPPO_20605) || is_project(OPPO_20630) || is_project(OPPO_20631) || is_project(OPPO_20632)
			|| is_project(OPPO_20633)|| is_project(OPPO_20634)|| is_project(OPPO_20635)|| is_project(OPPO_206B4)) {
				printk("use fan53870 pmic, not need to get regulator adress");
				regVCAMAF = NULL;
			} else {
		...
		if (is_project(OPPO_20601) || is_project(OPPO_20660) ||is_project(OPPO_20605)) {
			if (g_regVCAMAFEn == 0) {
				Cam_LDO_PowerOn_Fan53870(MAIN_AF_LDO_NUM, MAIN_AF_VOLTAGE);
				LOG_INF("20601 AF Power on by Fan53870 pmic");
				g_regVCAMAFEn = 1;
				usleep_range(5000, 5500);
			}
		}
                #endif
		/* Shipei.Chen@Cam.Drv, 20200526,  af porting for 20630!*/
		if (is_project(OPPO_20630) || is_project(OPPO_20631) || is_project(OPPO_20632)|| is_project(OPPO_20633)|| is_project(OPPO_20634)|| is_project(OPPO_20635)|| is_project(OPPO_206B4)) {
			if (g_regVCAMAFEn == 0) {
				Cam_LDO_PowerOn_Fan53870(ATHENSB_MAIN_AF_LDO_NUM, ATHENSB_MAIN_AF_VOLTAGE);
				LOG_INF("20630 AF Power on by Fan53870 pmic");
				g_regVCAMAFEn = 1;
				usleep_range(5000, 5500);
			}
		}
		...
		if (regVCAMAF != NULL && g_regVCAMAFEn == 1) {
			...
			g_regVCAMAFEn = 0;
                ...
                #ifdef CONFIG_MACH_MT6885
		/* Shipei.Chen@Cam.Drv, 20200422, Add for 20601 AF power on and down */
		if (is_project(OPPO_20601) || is_project(OPPO_20660) ||is_project(OPPO_20605)) {
			if (g_regVCAMAFEn == 1) {
				Cam_LDO_PowerDown_Fan53870(MAIN_AF_LDO_NUM);
				LOG_INF("20601 AF Power Down by Fan53870 pmic");
				g_regVCAMAFEn = 0;
			}
		}
                #endif
		...
		if (is_project(OPPO_20630) || is_project(OPPO_20631) || is_project(OPPO_20632)|| is_project(OPPO_20633)|| is_project(OPPO_20634)|| is_project(OPPO_20635)|| is_project(OPPO_206B4)) {
			if (g_regVCAMAFEn == 1) {
				Cam_LDO_PowerDown_Fan53870(ATHENSB_MAIN_AF_LDO_NUM);
				LOG_INF("20630 AF Power Down by Fan53870 pmic");
				g_regVCAMAFEn = 0;
			}
		}
	}
		     ...
		     unsigned long a_u4Param)
{
	...
	case AFIOC_G_OISPOSINFO:
		if (g_pstAF_CurDrv) {
			#ifdef VENDOR_EDIT
			/*Caohua.Lin@Camera.Drv,fix ois data flow issue,20200114*/
...
/* Feiping@Cam.Drv, 20190921, add for 19169 AF*/
	...
	//wenhui.chen@Cam.Drv, 20200402, add for  20601 lens
	/* Shipei.Chen@Cam.Drv, 20200526,  af porting for 20630!*/
	if (is_project(OPPO_19040) || is_project(OPPO_20601) || is_project(OPPO_20660) || is_project(OPPO_19131)
	||is_project(OPPO_19132) || is_project(OPPO_19133) || is_project(OPPO_19420) || is_project(OPPO_20611)
	|| is_project(OPPO_20610) || is_project(OPPO_20630) || is_project(OPPO_20631) || is_project(OPPO_20632)
	|| is_project(OPPO_20613) || is_project(OPPO_20605) || is_project(OPPO_20633) || is_project(OPPO_20634)
	|| is_project(OPPO_20635) || is_project(OPPO_20680) || is_project(OPPO_206B4)) {
		#if !defined(CONFIG_MTK_LEGACY)
			AFRegulatorCtrl(0);
			AFRegulatorCtrl(1);
		#endif
	/* Zhijian.Wang@Cam.Drv, 20200608,  af porting for 20625!*/
