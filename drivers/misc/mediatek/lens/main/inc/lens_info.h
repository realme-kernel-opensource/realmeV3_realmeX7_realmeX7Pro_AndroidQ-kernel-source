...
enum {
    ...
    MCU_CMD_OIS_CAL_DISABLE  = 0x1212,
    MCU_CMD_OIS_CAL_MODE     = 0x1213,
};

struct stAF_GyroOffsetResult{
	u8 result;
	int gyro_x_offset;
	int gyro_y_offset;
};
...
/* Structures */
#ifdef VENDOR_EDIT
//Feiping.Li@Cam.Drv, 20200119, add for more ois data
#define OIS_DATA_NUM 12
#else
...
#ifdef VENDOR_EDIT
//Feiping.Li@Cam.Drv, 20200111, add for ois engineer calibration
#define AFIOC_X_GETGYROOFFSET _IOWR(AF_MAGIC, 19, struct stAF_GyroOffsetResult)
#endif
