...
#include "gc02k0mipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc02k0_camera_sensor"
#define LOG_1 LOG_INF("GC02K0, MIPI 1LANE\n")
/****************************   Modify end    *******************************************/

#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __func__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);
kal_bool GC02K0DuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC02K0_SENSOR_ID,
...
#define DEVICE_VERSION_GC02K0     "gc02k0"
extern void register_imgsensor_deviceinfo(char *name, char *version, u8 module_id);
static kal_uint8 deviceInfo_register_value = 0x00;
#define MODULE_ID_OFFSET 0x0000
static kal_uint8 gGC02K0_SN[CAMERA_MODULE_SN_LENGTH];
static void read_eeprom_SN(void)
{
	kal_uint16 idx = 0;
	kal_uint8 *get_byte= &gGC02K0_SN[0];
	for (idx = 0; idx <CAMERA_MODULE_SN_LENGTH; idx++) {
		char pusendcmd[2] = {0x00 , (char)((0xB0 + idx) & 0xFF) };
		iReadRegI2C(pusendcmd , 2, (u8*)&get_byte[idx],1, 0xA4);
		LOG_INF("gGC02K0_SN[%d]: 0x%x  0x%x\n", idx, get_byte[idx], gGC02K0_SN[idx]);
	}
}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC02K0_MIRROR);
	...
	write_cmos_sensor(0x4e, GC02K0_BLK_Select_H);
	write_cmos_sensor(0x4f, GC02K0_BLK_Select_L);
	...
	write_cmos_sensor(0x92, GC02K0_STARTY);
	write_cmos_sensor(0x94, GC02K0_STARTX);
	...
	write_cmos_sensor(0x02, 0x84);//0x82
	...
	write_cmos_sensor(0x29, 0x06);//0x06
	write_cmos_sensor(0x2a, 0x12);//0x08
	write_cmos_sensor(0x2b, 0x08);//0x06
			...
			*sensor_id = return_sensor_id() & 0xffef;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if (deviceInfo_register_value == 0x00) {
					register_imgsensor_deviceinfo("Cam_r2", DEVICE_VERSION_GC02K0, imgsensor_info.module_id);
...
static kal_uint32 open(void)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	GC02K0DuringTestPattern = KAL_FALSE;
	return ERROR_NONE;
}
