...
#ifdef VENDOR_EDIT
/* Shengguang.Zhu@Cam.Drv, 20200416,  add the fan53870 for sensor porting! */
...
#define MAIN_AVDD_LDO_NUM (4)
#define MAIN_AVDD_VOLTAGE (2800)
#define MAIN_DVDD_LDO_NUM (1)
#define MAIN_DVDD_VOLTAGE (1050)

#define SUB_AVDD_LDO_NUM (6)
#define SUB_AVDD_VOLTAGE (2800)
#define SUB_DVDD_VOLTAGE (1100)

#define MAIN_TWO_AVDD_LDO_NUM (3)
#define MAIN_TWO_AVDD_VOLTAGE (2800)
#define MAIN_TWO_DVDD_LDO_NUM (2)
#define MAIN_TWO_DVDD_VOLTAGE (1100)
#endif
...
#ifdef VENDOR_EDIT
/* Shengguang.Zhu@Cam.Drv, 20200518,  add the fa53870 for sensor porting!*/
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
    ...
    /* Shengguang.Zhu@Cam.Drv, 20200416,  add the ldo3 for sensor porting! */
	if (is_project(OPPO_20630)|| is_project(OPPO_20631)|| is_project(OPPO_20632)|| is_project(OPPO_20633)|| is_project(OPPO_20634)|| is_project(OPPO_20635)|| is_project(OPPO_206B4))
	{
		if (pin == IMGSENSOR_HW_PIN_AVDD)
		{
			if (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN) {
				...
				if (pin_state == IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
					ret_mask = Cam_LDO_PowerDown_Fan53870(MAIN_TWO_AVDD_LDO_NUM);
				} else if ((pin_state > IMGSENSOR_HW_PIN_STATE_LEVEL_0)
					&& (pin_state < IMGSENSOR_HW_PIN_STATE_LEVEL_HIGH)) {
					ret_mask = Cam_LDO_PowerOn_Fan53870(MAIN_TWO_AVDD_LDO_NUM, MAIN_TWO_AVDD_VOLTAGE);
				}
			}
		...
		if (pin == IMGSENSOR_HW_PIN_DVDD)
		{
			...
			{
				if (pin_state == IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
					ret_mask = Cam_LDO_PowerDown_Fan53870(MAIN_DVDD_LDO_NUM);
				} else if ((pin_state > IMGSENSOR_HW_PIN_STATE_LEVEL_0)
					&& (pin_state < IMGSENSOR_HW_PIN_STATE_LEVEL_HIGH)) {
					ret_mask = Cam_LDO_PowerOn_Fan53870(MAIN_DVDD_LDO_NUM, MAIN_DVDD_VOLTAGE);
				}
			}
			else if (sensor_idx == IMGSENSOR_SENSOR_IDX_SUB)
			{
				if (pin_state == IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
					...
					&& (pin_state < IMGSENSOR_HW_PIN_STATE_LEVEL_HIGH)) {
					ret_mask = Cam_LDO_PowerOn_Fan53870(MAIN_DVDD_LDO_NUM, SUB_DVDD_VOLTAGE);
				}
			}
			else if (sensor_idx == IMGSENSOR_SENSOR_IDX_MAIN2)
			{
				if (pin_state == IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
					ret_mask = Cam_LDO_PowerDown_Fan53870(MAIN_TWO_DVDD_LDO_NUM);
				} else if ((pin_state > IMGSENSOR_HW_PIN_STATE_LEVEL_0)
					&& (pin_state < IMGSENSOR_HW_PIN_STATE_LEVEL_HIGH)) {
					ret_mask = Cam_LDO_PowerOn_Fan53870(MAIN_TWO_DVDD_LDO_NUM, MAIN_TWO_DVDD_VOLTAGE);
				}
			}
