...
#ifdef VENDOR_EDIT
	//hexiaoqiang@swdp.shanghai, 2020/04/07, Add gpufreq min limit interface
	if(g_min_limited_idx != NUM_OF_OPP_IDX - 1)
		request_idx = g_min_limited_idx - g_segment_max_opp_idx;
#endif
...
int mt_gpufreq_scene_protect(unsigned int min_freq, unsigned int max_freq)
{
	...
	if (min_freq > max_freq) {
		gpufreq_pr_info("@%s: GPU DVFS invalid input min_freq:%u max_freq:%u\n",
	...
	for (i = g_segment_max_opp_idx; i < g_max_opp_idx_num; i++) {
		...
		//min freq
		if (g_opp_table[g_max_opp_idx_num - 1 - i + g_segment_max_opp_idx].gpufreq_khz >= min_freq && !min_done) {
			g_limited_min_idx_array[IDX_SCENE_MIN_LIMITED] = g_max_opp_idx_num - 1 - i + g_segment_max_opp_idx;
			min_done = true;
		}
	}

	if (!max_done)
		g_limited_idx_array[IDX_SCENE_LIMITED] = g_segment_max_opp_idx;
	if (!min_done)
...
static int mt_gpufreq_var_dump_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "g_min_limited_idx = %d\n",
			g_min_limited_idx - g_segment_max_opp_idx);
#endif
...
/* Special SW setting */
	...
	if (strstr(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES,
						"k6853tv1") != NULL) {
#else
/* Hefeng.Wu@RM.MM.Display.Stability, 2020/08/14, modify for 5GB+ turbo GPU freq */
	if ((strstr(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES, "k6853tv1") != NULL)
            || (strstr(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES, "oppo6853_20630") != NULL)
            || (20613 == get_project()) || (20680 == get_project())) {
#endif
