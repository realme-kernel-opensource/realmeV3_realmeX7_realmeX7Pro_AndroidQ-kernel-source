...
//yangmingjin@realme.com, 2020/05/19, Add for power debug
#ifdef VENDOR_EDIT
		power_helper_save_to_buf("CCIF_MD wakeup source:(SRX_IDX/%d)(%u)",
			ccci_h->channel, md_ctrl->wakeup_count);
#endif /* VENDOR_EDIT */
	}
	ccci_hdr = *ccci_h;
...
//yangmingjin@realme.com, 2020/05/19, Add for power debug
#ifdef VENDOR_EDIT
			power_helper_save_to_buf("CCIF_MD wakeup source:(%d/%d/%x)(%u)\n",
...
int ccci_ccif_hif_init(unsigned char hif_id, unsigned char md_id)
{
	...
	md_ctrl->traffic_monitor.data = (unsigned long)md_ctrl;/* need review */
