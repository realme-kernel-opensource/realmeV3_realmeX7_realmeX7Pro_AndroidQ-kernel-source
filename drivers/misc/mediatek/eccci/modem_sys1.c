...
static irqreturn_t md_cd_ccif_isr(int irq, void *data)
{
	...
	if (md_info->channel_id & (1<<AP_MD_SEQ_ERROR)) {
		...
		md->ops->dump_info(md, DUMP_FLAG_CCIF | DUMP_FLAG_MD_WDT, NULL, 0);
	}
...
static inline int md_sys1_sw_init(struct ccci_modem *md)
{
	...
	ret = request_irq(md_info->ap_ccif_irq_id, md_cd_ccif_isr,
		md_info->ap_ccif_irq_flags | IRQF_NO_SUSPEND, "CCIF0_AP", md);
	if (ret) {
		CCCI_ERROR_LOG(md->index, TAG,
...
static int md_cd_force_assert(struct ccci_modem *md, enum MD_COMM_TYPE type)
{
	...
	if (type == CCIF_INTERRUPT)
		...
		md->ops->dump_info(md, DUMP_FLAG_CCIF_REG  | DUMP_FLAG_MD_WDT, NULL, 0);
	}
	return 0;
}
	...
	const char *buf, size_t count)
{
	...
	if (md_state != GATED && md_state != INVALID) {
		...
		if (strncmp(buf, "smem_exp", count-1) == 0)
			md->ops->dump_info(md, DUMP_FLAG_SMEM_EXP | DUMP_FLAG_MD_WDT, NULL, 0);
		if (strncmp(buf, "smem_ccism", count-1) == 0)
			md->ops->dump_info(md, DUMP_FLAG_SMEM_CCISM | DUMP_FLAG_MD_WDT, NULL, 0);
		if (strncmp(buf, "smem_ccb_ctrl", count-1) == 0)
			md->ops->dump_info(md,
				DUMP_FLAG_SMEM_CCB_CTRL | DUMP_FLAG_MD_WDT, NULL, 0);
		if (strncmp(buf, "smem_ccb_data", count-1) == 0)
			md->ops->dump_info(md,
				DUMP_FLAG_SMEM_CCB_DATA | DUMP_FLAG_MD_WDT, NULL, 0);
		if (strncmp(buf, "pccif", count - 1) == 0)
			md->ops->dump_info(md, DUMP_FLAG_PCCIF_REG | DUMP_FLAG_MD_WDT, NULL, 0);
		if (strncmp(buf, "image", count - 1) == 0)
			md->ops->dump_info(md, DUMP_FLAG_IMAGE | DUMP_FLAG_MD_WDT, NULL, 0);
		if (strncmp(buf, "layout", count - 1) == 0)
			md->ops->dump_info(md, DUMP_FLAG_LAYOUT | DUMP_FLAG_MD_WDT, NULL, 0);
		if (strncmp(buf, "mdslp", count - 1) == 0)
			md->ops->dump_info(md, DUMP_FLAG_SMEM_MDSLP | DUMP_FLAG_MD_WDT, NULL, 0);
