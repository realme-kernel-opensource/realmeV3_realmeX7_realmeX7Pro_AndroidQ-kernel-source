	...
	(struct mt6885_spm_wake_status *wakesta)
{
	...
	for (i = 0; i < IRQ_NUMBER; i++) {
		...
		if (mt6885_spm_wakesrc_irqs[i].wakesrc & wakesta->r12) {
			...
			if (mt_irq_get_pending(irq_no)) {
				log_wakeup_reason(irq_no);
				if(mt6885_spm_wakesrc_irqs[i].wakesrc == WAKE_SRC_STA1_EINT_EVENT_B) {//mt6358-irq
					is_pmic_mt6358_wakeup_reason = 1;
				}
			}
			#endif
		}
...
//yangmingjin@realme.com, 2020/05/19, Add for power debug
#ifdef VENDOR_EDIT
int mt6885_wakeup_r13_table_size;
...
//yangmingjin@realme.com, 2020/05/19, Add for power debug
#ifdef VENDOR_EDIT
	power_helper_r13_sleep_data_init(mt6885_wakeup_r13_table, mt6885_wakeup_r13_table_size);
#endif /* VENDOR_EDIT */
