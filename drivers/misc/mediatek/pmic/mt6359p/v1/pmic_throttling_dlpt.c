...
#ifdef VENDOR_EDIT
//cuixiaogang@SH, 2019/11/26. disable low battery limit
	...
	do {
		...
		if (cur_ui_soc <= DLPT_POWER_OFF_THD) {
			if (dlpt_check_power_off() == 1) {
				 ...
				 */
#ifndef VENDOR_EDIT
/* LiYue@BSP.CHG.Basic, 2019/09/23, Delete for remove dlpt shutdown */
				if (power_off_cnt >= 4 &&
...
static void pmic_uvlo_init(void)
{
	...
	pr_info("POWER_UVLO_VOLT_LEVEL_03 = %d, RG_UVLO_VTHL = 0x%x\n"
		, POWER_UVLO_VOLT_LEVEL
