...
#ifdef VENDOR_EDIT
/*RM.BSP.storage.sdcard add for distinguish zola and athen-b*/
...
/* #ifdef VENDOR_EDIT*/
/*Yaqiang.Shi@RM.MM.DisplayDriver.LCD  while need use ldo5,need to disable sdcard_det_en */
	.sdcard_det_en = false,
/*else*/
	//.sdcard_det_en = true,
/*endif*/
        .sdcard_det_High =true,
};
...
#ifndef VENDOR_EDIT
/*WeiZhenXi RM.BSP.storage.sdcard add for distinguish zola and athen-b*/
...
#ifdef VENDOR_EDIT
/*WeiZhenXi RM.BSP.storage.sdcard add for distinguish zola and athen-b*/
	if (id == MT6360_LDO_LDO5 && (get_project()==20611 || get_project()==20610) ) {
		dev_err(&rdev->dev,
			"%s: zola mt6360_ldo_enable \n", __func__);
#else
				  ...
				  struct mt6360_ldo_platform_data *pdata)
{
	...
	if (ret < 0)
        ...
        if(pdata->sdcard_det_High ==1)
                ret = mt6360_ldo_reg_update_bits(mli,MT6360_LDO_LDO5_EN_CTRL1 + 2, ldo_ctrl_mask[2], 0x01);
                if (ret < 0){
                                dev_err(mli->dev,"%s: set ldo5_ctrls fail (%d)\n", __func__,ret);
                                return ret;
                }
...
static const struct mt6360_val_prop mt6360_val_props[] = {
        MT6360_DT_VALPROP(sdcard_det_High, struct mt6360_ldo_platform_data),
};
