...
#endif
static int mt6360_set_enter_shipping_mode(struct charger_device *chg_dev);
/* ================== */
/* Internal Functions */
/* ================== */
...
/* Kaili.Lu@BSP.CHG.Basic, 2019/09/30,  Add for HVDCP charging */
		if (true == is_mtksvooc_project) {
			...
			if (mpci->hvdcp_cfg_9v_done && (mpci->hvdcp_detach_time - mpci->hvdcp_detect_time <= OPPO_HVDCP_DETECT_TO_DETACH_TIME)) {
				...
				//dev_err(mpci->dev, "%s: HVDCP_EXIT_ABNORMAL \n", __func__);

			} else {
				mpci->hvdcp_exit_stat = HVDCP_EXIT_NORMAL;
				//dev_err(mpci->dev, "%s: HVDCP_EXIT_NORMAL \n", __func__);
			}
...
#ifdef VENDOR_EDIT
//#Dongru.Zhao@BSP.CHG.Basic, 2019/12/16, zdr Add for 19151 ship_mode .

	/* Ship Mode */
	.enable_ship_mode = mt6360_set_enter_shipping_mode,
...
enable_hvdcp:
	dev_err(oppompci->dev, "%s: enable hvdcp \n", __func__);
	ret = mt6360_pmu_reg_write(oppompci->mpi, HVDCP_DEVICE_TYPE, 0x0);
	if (ret < 0)
