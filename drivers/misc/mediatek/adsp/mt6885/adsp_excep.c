...
static void adsp_exception_dump(struct adsp_exception_control *ctrl)
{
	...
	/* #ifdef VENDOR_EDIT hongyu.lu@BSP.Kernel.Driver, 2020/02/27, Add for modify crash */
	if( n < ADSP_AED_STR_LEN) {
	...
	if( n < ADSP_AED_STR_LEN) {
		coredump->task_name[15] = 0x00;
		n += snprintf(detail + n, ADSP_AED_STR_LEN - n,
		      "CRDISPATCH_KEY:ADSP exception/%s\n",
		      coredump->task_name);
	}
	if( n < ADSP_AED_STR_LEN) {
	coredump->assert_log[511] = 0x00;
	n += snprintf(detail + n, ADSP_AED_STR_LEN - n, "%s",
		      coredump->assert_log);
	}
	detail[ADSP_AED_STR_LEN - 1] = 0x00;
	/* #endif VENDOR_EDIT */
	pr_info("%s", detail);
