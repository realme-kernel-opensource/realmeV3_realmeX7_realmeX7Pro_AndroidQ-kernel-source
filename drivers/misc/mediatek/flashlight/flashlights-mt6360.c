...
extern unsigned int mt6360_timeout_ms[MT6360_CHANNEL_NUM];
#else
...
#ifdef CONFIG_FLASHLIGHT_COMPATIBLE
extern int mt6360_current[MT6360_LEVEL_NUM];
#else
static const int mt6360_current[MT6360_LEVEL_NUM] = {
...
#ifdef CONFIG_FLASHLIGHT_COMPATIBLE
int mt6360_is_charger_ready(void)
{
...
EXPORT_SYMBOL(mt6360_is_charger_ready);/* Shengguang.Zhu@Cam.Drv, 20200612,  add for flashlight support MT6360&AW3642*/
#else
...
#ifdef CONFIG_FLASHLIGHT_COMPATIBLE
int mt6360_verify_level(int level)
{
	if (level < 0)
		level = 0;
...
EXPORT_SYMBOL(mt6360_verify_level);/* Shengguang.Zhu@Cam.Drv, 20200612,  add for flashlight support MT6360&AW3642*/
...
static int mt6360_set_level_ch1(int level)
{
	...
	if (!mt6360_is_torch(level))
		flashlight_set_torch_brightness(
				flashlight_dev_ch1, mt6360_torch_level[level]);
	#endif
	printk("enter set_strobe_brightness");
...
#ifdef CONFIG_FLASHLIGHT_COMPATIBLE
int mt6360_set_level(int channel, int level)
{
	if (channel == MT6360_CHANNEL_CH1)
...
EXPORT_SYMBOL(mt6360_set_level);/* Shengguang.Zhu@Cam.Drv, 20200612,  add for flashlight support MT6360&AW3642*/
#else
...
EXPORT_SYMBOL(mt6360_set_scenario);
#else
 ...
 *****************************************************************************/
int mt6360_operate(int channel, int enable)
{
	ktime_t ktime;
	unsigned int s;
	unsigned int ns;
...
EXPORT_SYMBOL(mt6360_operate);/* Shengguang.Zhu@Cam.Drv, 20200612,  add for flashlight support MT6360&AW3642*/
#else
/******************************************************************************
 * Flashlight operation wrapper function
 *****************************************************************************/
...
static int mt6360_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_TIME_OUT_TIME_MS:
		...
		//wenhui.chen@Cam.Drv, 20200402, add for  20601 flash
