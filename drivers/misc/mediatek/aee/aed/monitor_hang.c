...
#endif
static int key_process_pid; //mtk71029 update for common hang detect.
...
#define KPOC_CHARGER "kpoc_charger"

typedef struct hang_detect_mode {
	enum boot_mode_t boot_mode;
	const char * process_name;
} hang_detect_mode;

static struct hang_detect_mode hang_detect_mode_table[]={
	{NORMAL_BOOT, SYSTEM_SERVER},
	{ALARM_BOOT, SYSTEM_SERVER},
	{KERNEL_POWER_OFF_CHARGING_BOOT, KPOC_CHARGER},
	{UNKNOWN_BOOT, SYSTEM_SERVER},
};

static struct hang_detect_mode select_hang_mode;

static void get_hang_detect_mode() {
	enum boot_mode_t boot_mode = get_boot_mode();
	int find = 0;
	int i;
	for (i = 0; i < sizeof(hang_detect_mode_table) / sizeof (hang_detect_mode) ; i++) {
		if (boot_mode == hang_detect_mode_table[i].boot_mode) {
			find = 1;
			select_hang_mode.boot_mode = boot_mode;
			select_hang_mode.process_name = hang_detect_mode_table[i].process_name;
		}
	}
	//default set key process is system_server
	if (find == 0) {
		select_hang_mode.boot_mode = boot_mode;
		select_hang_mode.process_name = SYSTEM_SERVER;
	}
	pr_info("[hang_detect] boot_mode=%d, key process=%s.\n", select_hang_mode.boot_mode, select_hang_mode.process_name);
}
//mtk71029 add end.
...
static int FindTaskByName(const char *name)
{
...
#ifndef __aarch64__	 /* 32bit */
		if (strcmp(t->comm, select_hang_mode.process_name) == 0)
			dump_native = 1;
		else
			dump_native = 0;
#else
...
static void hang_dump_backtrace(void)
{
	...
	for_each_process(p) {
		...
		if ((strcmp(p->comm, "surfaceflinger") == 0) ||
			...
			(strcmp(p->comm, KPOC_CHARGER) == 0) ) { //mtk71029 add kpoc_charger
...
static int hang_detect_thread(void *arg)
{
	...
	sched_setscheduler(current, SCHED_FIFO, &param);
	reset_hang_info();
	msleep(90 * 1000); //mtk71029 update to 90s
...
#else
		key_process_pid = FindTaskByName(select_hang_mode.process_name);
		if (reboot_flag || ((hd_detect_enabled == 1) &&
			(key_process_pid != -1)))
#endif
		{
...
void aee_kernel_RT_Monitor_api(int lParam)
{
	...
	if (lParam == 0) {
		...
		if (hd_timeout < 2) { //mtk71029 update to 1min.
			/* hang detect min timeout is 2 (1min) */
