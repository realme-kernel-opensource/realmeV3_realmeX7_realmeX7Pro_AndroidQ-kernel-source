...
static struct LCM_setting_table lcm_dc_enter[] = {
	{REGFLAG_CMD,3,{0xF0, 0x5A, 0x5A}},
...
static struct LCM_setting_table lcm_dc_exit[] = {
	{REGFLAG_CMD,3,{0xF0, 0x5A, 0x5A}},
	{REGFLAG_CMD,2,{0xB0,0x1E}},
	{REGFLAG_CMD,8,{0xB3,0x05, 0x04, 0x03, 0x02, 0x01, 0x00, 0x00}},
	{REGFLAG_CMD,3,{0xF0, 0xA5, 0xA5}},
	{REGFLAG_END_OF_TABLE, 0x00, {}}
};
...
static int oppo_lcm_dc_enterd(void *dsi, dcs_write_gce cb,void *handle)
{
	int i;

	pr_err("debug for lcm %s,oppo_dc_enable = %d\n", __func__,oppo_dc_enable);
	for (i = 0; i < sizeof(lcm_dc_enter)/sizeof(lcm_dc_enter[0]); i++){
		cb(dsi, handle, lcm_dc_enter[i].para_list, lcm_dc_enter[i].count);
	}
	return 0;
}

static int oppo_lcm_dc_exitd(void *dsi, dcs_write_gce cb,void *handle)
{
	int i;

	pr_err("debug for lcm %s,oppo_dc_enable = %d\n", __func__,oppo_dc_enable);
	for (i = 0; i < sizeof(lcm_dc_exit)/sizeof(lcm_dc_exit[0]); i++){
		cb(dsi, handle, lcm_dc_exit[i].para_list, lcm_dc_exit[i].count);
	}
	return 0;
}
...
//	.aod = enter_aod,
	...
	.lcm_dc_enterd = oppo_lcm_dc_enterd,
	.lcm_dc_exitd = oppo_lcm_dc_exitd,
