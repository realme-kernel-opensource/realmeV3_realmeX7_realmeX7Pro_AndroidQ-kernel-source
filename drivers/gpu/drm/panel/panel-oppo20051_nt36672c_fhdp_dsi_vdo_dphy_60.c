...
static struct regulator *disp_ldo2;

static int lcm_panel_ldo2_regulator_init(struct device *dev)
{
	static int regulator_inited;
	int ret = 0;
	...
	if (regulator_inited)
		return ret;
    pr_err("get lcm_panel_ldo2_regulator_init\n");

	/* please only get regulator once in a driver */
	disp_ldo2 = regulator_get(dev, "vtp");
	if (IS_ERR(disp_ldo2)) { /* handle return value */
		ret = PTR_ERR(disp_ldo2);
		pr_err("lcd get ldo2 fail, error: %d\n", ret);
		return ret;
	}
	regulator_inited = 1;
	return ret; /* must be 0 */
...
static int lcm_panel_ldo2_enable(struct device *dev)
{
	int ret = 0;
	int retval = 0;

	lcm_panel_ldo2_regulator_init(dev);

	/* set voltage with min & max*/
	ret = regulator_set_voltage(disp_ldo2, 1800000, 1800000);
	if (ret < 0)
		pr_err("set voltage disp_ldo2 fail, ret = %d\n", ret);
	retval |= ret;

	/* enable regulator */
	ret = regulator_enable(disp_ldo2);
	if (ret < 0)
		pr_err("enable regulator disp_ldo2 fail, ret = %d\n", ret);
	retval |= ret;
    pr_err("get lcm_panel_ldo2_enable\n");

	return retval;
}

static int lcm_panel_ldo2_disable(struct device *dev)
{
	int ret = 0;
	int retval = 0;
	...
	ret = regulator_disable(disp_ldo2);
	if (ret < 0)
		pr_err("lcd disable regulator disp_ldo2 fail, ret = %d\n", ret);
	retval |= ret;
    pr_err("disable regulator disp_ldo2\n");
...
static int lcm_unprepare(struct drm_panel *panel)
{
	...
	msleep(10);
	lcm_panel_ldo2_disable(ctx->dev);
