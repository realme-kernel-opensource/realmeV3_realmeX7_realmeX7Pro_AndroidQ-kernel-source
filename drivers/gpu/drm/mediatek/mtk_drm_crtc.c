...
/* shangruofan@PSW.MM.DisplayDriver.DC, 2020/7/28, add for DC enter open elvss fix */
	if(oppo_dc_enable)
		comp->funcs->io_cmd(comp, cmdq_handle, DC_SETTING_ENTER, &level);
#endif
	/* set backlight */
	if (comp->funcs && comp->funcs->io_cmd)
		comp->funcs->io_cmd(comp, cmdq_handle, DSI_SET_BL, &level);
...
/* shangruofan@PSW.MM.DisplayDriver.DC, 2020/7/28, add for DC enter open elvss fix */
	if(!oppo_dc_enable)
		comp->funcs->io_cmd(comp, cmdq_handle, DC_SETTING_EXIT, &level);
#endif
	return 0;
}

int oppo_mtk_dc_backlight_exit(struct drm_crtc *crtc)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct cmdq_pkt *cmdq_handle;
	struct mtk_ddp_comp *comp = mtk_ddp_comp_request_output(mtk_crtc);
...
int mtk_drm_aod_setbacklight(struct drm_crtc *crtc, unsigned int level)
{
	...
	if (mtk_crtc->enabled && !mtk_state->prop_val[CRTC_PROP_DOZE_ACTIVE]) {
...
/** LiPing-M@PSW.MM.Display.LCD.Machine, 2019/11/15,add for dc backlight **/
		if ((exit_dc_flag == 1) && (crtc_id == 0)) {
			oppo_mtk_dc_backlight_exit(crtc);
			exit_dc_flag = 0;
		}
...
#ifdef VENDOR_EDIT
	if (priv->num_pipes == 1) {
		mtk_ddp_comp_io_cmd(output_comp, NULL, GET_PANEL_NAME,
                                    &panel_name);
		if (!strcmp(panel_name, "nt36672c_fhdp_dsi_vdo_dphy_jdi_lcm_drv")) {
			register_device_proc("lcd", "NT36672C_JDI", "nt_jdi_4096");
		} else if ((!strcmp(panel_name, "oppo20611_nt36672c_tm_fhdp_dsi_vdo_dphy"))
		|| (!strcmp(panel_name, "nt36672c_fhdp_dsi_vdo_dphy_jdi_lcm_drv_120"))) {
			register_device_proc("lcd", "NT36672C_TM", "hx_tm_4096");
		} else if ((!strcmp(panel_name, "oppo19040_samsung_ana6705_1080p_dsi_cmd"))
                || (!strcmp(panel_name, "oppo20601_samsung_sofe03f_m_1080p_dsi_cmd"))) {
			register_device_proc("lcd", "AMS644VA04_MTK04", "samsung1024");
		} else if ((!strcmp(panel_name, "oppo20630_s68fc01_lcm"))
                || (!strcmp(panel_name, "hx83112f_fhdp_dsi_vdo_dphy_tianma_lcm_drv"))) {
                        register_device_proc("lcd", "AMS644VA04_MTK04", "samsung4096");
		} else if ((!strcmp(panel_name, "nt36672c_dsi_vdo_dphy_jdi_lcm_drv"))) {
			register_device_proc("lcd", "NT3625B_JDI", "nt_jdi_4096");
		} else if ((!strcmp(panel_name, "oppo20611_hx83112f_fhdp_dsi_vdo_dphy_tianma_lcm_drv"))) {
                        register_device_proc("lcd", "HX83112_TM", "tm_4096");
		}
	}
	/*
