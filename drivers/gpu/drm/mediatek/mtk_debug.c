...
#endif
#ifndef VENDOR_EDIT
/* liwei.a@PSW.MM.DisplayDriver.Perf, 2019/10/31, add for shutdown ufs log defaultly*/
bool g_mobile_log=1;
bool g_fence_log;
bool g_irq_log=1;
bool g_detail_log=1;
...
extern int mtk_drm_aod_setbacklight(struct drm_crtc *crtc, unsigned int level);
...
int oppo_set_hbm(unsigned int hbm_mode)
{
	struct drm_crtc *crtc;
	...
	oppo_mtk_drm_sethbm(crtc, hbm_mode);

	return 0;
}
EXPORT_SYMBOL(oppo_set_hbm);

/*liwei.a@PSW.MM.LCD.Display.Stability, 2019/10/10, add debug interface for mipi clk change*/
/*int mtk_crtc_mipi_freq_switch(struct drm_crtc *crtc, int en,
			unsigned int userdata);*/
int oppo_mipi_clk_change(unsigned int level)
{
	struct drm_crtc *crtc;
	...
	mtk_crtc_mipi_freq_switch(crtc, level, 1);

	return 0;
}
EXPORT_SYMBOL(oppo_mipi_clk_change);
#endif
...
static void process_dbg_opt(const char *opt)
{
	...
	if (strncmp(opt, "helper", 6) == 0) {
	...
	} else if (strncmp(opt, "meta_color:", 11) == 0) {
		struct drm_crtc *crtc;
		int ret;
		char cmd;

		ret = sscanf(opt, "meta_color:0x%x\n", &cmd);
		if (ret != 1) {
			DDPPR_ERR("%d error to parse cmd %s\n",
				__LINE__, opt);
			return;
		...
		_primary_path_idle_for_meta_mode(crtc, cmd);
		...
		for (i = 0; i < size; i++)
			DDPPR_ERR(" return value: para[%d] = 0x%x\n", i, para[i]);
	} else if (strncmp(opt, "hbm:", 4) == 0) {
		unsigned int hbm_mode;
		int ret;

		ret = sscanf(opt, "hbm:%u\n", &hbm_mode);
		if (ret != 1) {
			DDPPR_ERR("%d error to parse cmd %s\n",
				__LINE__, opt);
			return;
		}
		oppo_set_hbm(hbm_mode);
	} else if (strncmp(opt, "chg_mipi_clk:", 13) == 0) {
		unsigned int level;
		int ret;

		ret = sscanf(opt, "chg_mipi_clk:%u\n", &level);
		if (ret != 1) {
			DDPPR_ERR("%d error to parse cmd %s\n",
				__LINE__, opt);
			return;
		}
		oppo_mipi_clk_change(level);
