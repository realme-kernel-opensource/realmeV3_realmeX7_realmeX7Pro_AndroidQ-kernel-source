...
extern unsigned int doze_area_count;
bool doze_area_flag = 0;
#endif
		...
		const char *buf, size_t count)
{
	...
	aod_area_set_flag = 1;
	#ifdef OPPO_DISPLAY_LCD_20630
	if ((doze_area_count == 1) && (area_count == 2)) {
		doze_area_flag = 1;
		doze_area_count = 0;
		area_count = 0;
	}
...
/*	if (cabc_mode == 0) {
	...
	//if(tmp != hbm_mode) {
		oppo_mtk_drm_setcabc(crtc, cabc_mode);
	//}
	if (cabc_mode != cabc_back_flag) cabc_mode = cabc_back_flag;
