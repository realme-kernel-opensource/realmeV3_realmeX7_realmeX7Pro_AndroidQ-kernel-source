...
static int fsa4480_usbc_analog_setup_switches(struct fsa4480_priv *fsa_priv)
{
	...
	if (fsa_priv->plug_state) {
		...
		if (gpio_is_valid(fsa_priv->hs_det_pin)) {
            ...
            if (strstr(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES, "oppo6853_20630") != NULL) {
                ...
                pr_info("do set gpio low 1\n");
                hs_det_gpio = pinctrl_lookup_state(pinctrl1, "hs_det_gpio_low");
		...
		/* Hefeng.Wu@RM.MM.AudioDriver.HeadsetDet, 2020/08/26, add for connect SENSE to GND when
		 * headset plug out */
