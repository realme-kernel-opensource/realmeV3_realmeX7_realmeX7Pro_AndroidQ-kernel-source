...
#include "../../codecs/aw87339/k6873v1_64_alpha/aw87339.h"
#endif
...
#ifdef CONFIG_SND_SOC_AW87339
/*An.Hu@PSW.MM.AudioDriver.Machine,2020/02/25, Add for aw87339*/
				       ...
				       int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		/* spk amp on control */
#ifdef VENDOR_EDIT
        /* Hefeng.Wu@RM.MM.AudioDriver.Machine, 2020/06/13, add for speaker bringup */
        if ((20625 == get_project()) || (20626 == get_project())) {
            mt6853_afe_gpio_request(&afe, true, EXT_SPK_AMP_ENABLE,0);
        }
...
/* Wenyang.Fan@PSW.MULTIMEDIA.AUDIODRIVER.MACHINE,2020/06/16, Add for adjust codec & Sia PA sequence */
	...
	case SND_SOC_DAPM_PRE_PMD:
        ...
        /* Hefeng.Wu@RM.MM.AudioDriver.Machine, 2020/06/13, add for speaker bringup */
        if ((20625 == get_project()) || (20626 == get_project())) {
            mt6853_afe_gpio_request(&afe, false, EXT_SPK_AMP_ENABLE,0);
        }
...
static int mt6853_mt6359_dev_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < card->num_links; i++) {
		 ...
		 * add for tfa9874 */
		if (!strcmp(mt6853_mt6359_dai_links[i].name, "I2S3") || !strcmp(mt6853_mt6359_dai_links[i].name, "I2S0")) {
			dev_info(&pdev->dev, "%s: codec_node no use for %s\n", __func__, mt6853_mt6359_dai_links[i].name);
			continue;
		}
#endif /* CONFIG_SND_SOC_TFA9874 */
#endif /* VENDOR_EDIT */
