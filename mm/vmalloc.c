...
/* Kui.Zhang@tech.kernel.mm, 2020-02-26, sort the vmalloc stack called times.
 */
...
#if defined(VENDOR_EDIT) && defined(CONFIG_VMALLOC_DEBUG)
/* Kui.Zhang@tech.kernel.mm, 2020-02-26, save vmalloc stack. */
...
static noinline ml_depot_stack_handle_t save_vmalloc_stack(gfp_t flags)
{
...
/* Kui.Zhang@tech.kernel.mm, 2020-02-13, add new proc interface for create
 * vmalloc debug enable or disable.
 */
	...
	const char __user *buff, size_t len, loff_t *ppos)
{
	...
	ret = kstrtol(kbuf, VD_VALUE_LEN-2, &val);
	if (ret)
		return -EINVAL;
...
{
#if defined(VENDOR_EDIT) && defined(CONFIG_VMALLOC_DEBUG)
	/* Kui.Zhang@tech.kernel.mm, 2020-02-26, save vmalloc called stack. */
	unsigned int handle = 0;
	if (flags & VM_ALLOC) {
		atomic_inc(&vmalloc_count);
		if (vmalloc_debug_enable)
			handle = save_vmalloc_stack(GFP_KERNEL);
	}
#endif
	spin_lock(&vmap_area_lock);
	vm->flags = flags;
	...
	vm->caller = caller;
#if defined(VENDOR_EDIT) && defined(CONFIG_VMALLOC_DEBUG)
	/* Kui.Zhang@tech.kernel.mm, 2020-02-26, save stach hash*/
	vm->hash = handle;
#endif
	va->vm = vm;
	va->flags |= VM_VM_AREA;
...
#if defined(VENDOR_EDIT) && defined(CONFIG_VMALLOC_DEBUG)
		/* Kui.Zhang@tech.kernel.mm, 2020-02-26, update the count while
		 * vfree. */
...
#endif
#if defined(VENDOR_EDIT) && defined(CONFIG_VMALLOC_DEBUG)
/* Kui.Zhang@tech.kernel.mm, 2020-02-26, implement vmalloc_debug proc file
 * operations. */
typedef struct {
	const void *caller;
	unsigned int hash;
