...
/* Kui.Zhang@tech.kernel.mm, 2020-02-13, operation on kmalloc_debug_cache, need
 * atomic operatton apis.
 */
...
/* Kui.Zhang@tech.kernel.mm, 2020-02-13, kmalloc_debug_caches store the
 * kmalloc caches with debug flag.
 */
...
#if defined(VENDOR_EDIT) && defined(CONFIG_KMALLOC_DEBUG)
/* Kui.Zhang@tech.kernel.mm, 2020-02-13, kmalloc_debug_info add debug to
 * slab name.
 */
...
#if defined(VENDOR_EDIT) && defined(CONFIG_KMALLOC_DEBUG)
/* Kui.Zhang@tech.kernel.mm, 2020-02-13, new api */
	...
	const char __user *buff, size_t len, loff_t *ppos)
{
	...
	ret = kstrtol(kbuf, KD_VALUE_LEN-2, &val);
	if (ret)
		return -EINVAL;
			...
			char __user *buffer, size_t count, loff_t *off)
{
	...
	for (i = 0; i <= KMALLOC_SHIFT_HIGH; i++) {
		...
		if (len >= 511)
			break;
		len += scnprintf(&kbuf[len], 511 - len, "%-8lu %lu\n",
