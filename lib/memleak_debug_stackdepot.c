...
static void **stack_slabs = NULL;

static int depot_index;
static int next_slab_inited;
static size_t depot_offset;
static DEFINE_SPINLOCK(depot_lock);
static DEFINE_MUTEX(depot_init_mutex);
static atomic_t stack_depot_inited = ATOMIC_INIT(0);
...
static bool init_stack_slab(void **prealloc)
{
	...
	if (stack_slabs[depot_index] == NULL) {
		 ...
		 * |next_slab_inited| above and in ml_depot_save_stack().
		 */
		...
		u32 hash, void **prealloc, gfp_t alloc_flags)
{
	...
	if (unlikely(depot_offset + required_size > STACK_ALLOC_SIZE)) {
		 ...
		 * |next_slab_inited| in ml_depot_save_stack() and
		 * init_stack_slab().
		 */
...
static struct stack_record **stack_table = NULL;
...
void ml_depot_fetch_stack(ml_depot_stack_handle_t handle, struct stack_trace *trace)
{
	...
	if (atomic_read(&stack_depot_inited) == 0) {
		pr_err("stack_depot_inited is not inited\n");
		return;
	}
...
int ml_depot_init(void)
{
	...
	mutex_lock(&depot_init_mutex);
	inited = atomic_read(&stack_depot_inited);
	if (inited != 0) {
		mutex_unlock(&depot_init_mutex);
		pr_err("stack_depot_inited is inited, val %d\n", inited);
		return 0;
	}

	size = sizeof(void *) * STACK_ALLOC_MAX_SLABS;
	stack_slabs = (void **)vmalloc(size);
	if (!stack_slabs) {
		mutex_unlock(&depot_init_mutex);
		pr_err("vmalloc stack_slabs %d failed.\n", size);
		return -1;
	}
	memset(stack_slabs, 0, size);

	size = sizeof(struct stack_record *) * STACK_HASH_SIZE;
	stack_table = (struct stack_record **)vmalloc(size);
	if (!stack_table) {
		mutex_unlock(&depot_init_mutex);
		pr_err("vmalloc stack_table %d failed.\n", size);
		vfree(stack_slabs);
		stack_slabs = NULL;
		return -1;
	}
	...
	atomic_set(&stack_depot_inited, 1);
	mutex_unlock(&depot_init_mutex);
	return 0;
}
